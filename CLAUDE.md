...
13. **land / verify 紀律(AI-friendly:宣告前先查證)**:
    ...
    - **land pipeline**:off `origin/main` 開分支 → 隔離 worktree、標死 scope → 親驗全 diff(無 scope-creep)+ 親讀關鍵邏輯碼 → `bash bin/ci.sh` **rc == 0 且最後一行**精確為 `[ci] all green` → FF fast-forward push(`git push origin <b>:main`)→ 雙源查證(`git ls-remote` + `gh api .../commits/main --jq .sha`)→ autodeploy 盯 `/api/version` git_sha(公開免 auth;version 欄恆 0.0.0,對賬看 git_sha)→ 收退路(`worktree remove` + `branch -D`)。

...
4. **治理層 = {owner, admin_agent} only**(方案 B 語意面已落地):改 global context / role、mint 身分 token、bootstrap(mint member JWT 的 seam)這類動作,只有 **owner 或 admin agent** 能執行。⚠️ **lessons 是分級的、不是整條鎖治理層**(owner 2026-07-27 `rc-46599297a1c4` 拍板:**程式現況是對的,改文件對齊**;此前本條把 lessons 與 global context / role 並列,寫窄了):**每個 agent 都可以整理自己那個 role 的 lessons**(`lessonsWriteAuthz` 的 self-role 規則,身分取自 verified sub 的名冊列、不是請求欄位),寫**別人** role 的才需要 admin 以上;READ 不設此限。⚠️ 這句預設了「**有** role」:比對的是名冊列的 `role_key` 對上路徑的 `role_key`,而路徑那個永遠非空,所以**名冊列沒有 `role_key` 的 caller 一個 role 都寫不了**(不是「只能寫自己的」,是零)——外包 worker（`ow-` 是 `member` 表中 `kind=outsource` 的列；`memberFromWorker` 固定其 `role_key=""`）與任何 `role_key=""` 的成員都落在這裡。admin agent = **agent-scope token 且其 member `role_key=="assistant"`**(耐久角色判定,非 hardcode id;seeded Mira 是第一個實例——舊「sub==mira」hardcode 判法已廢,split-brain 統一到 role 側,見 §14 / `docs/design/caller-identity-convention.md`)。⚠️ **warden 不是治理 principal**(machine class 在階梯最底——warden 零自決 auth,見 §6)。agent 不能自我提權呼叫治理端點;每條路由必須在表上宣告明確 principal 門檻,不靠「agent 不知端點存在」當安全邊界。**閉環**:bootstrap 同鎖 admin 以上,否則任意 agent token 可 `bootstrap {member_id:"mira"}` 拿 admin token 繞過治理 choke;hire 的 kind/role_key 提權面同鎖 admin 以上(否則 agent 可自僱一個 assistant)。
...
13. **land / verify 紀律(AI-friendly:宣告前先坐實)**:
    ...
    - **land pipeline**:off `origin/main` 開分支 → 隔離 worktree、標死 scope → 親驗全 diff(無 scope-creep)+ 親讀關鍵邏輯碼 → `bash bin/ci.sh` **rc == 0 且最後一行**精確為 `[ci] all green` → push 分支 → **開 PR**(`gh pr create`)→ 等 `cloud-gates` 通過(約 6 分鐘;**`BLOCKED` 是還在跑、不是失敗**)→ `gh pr merge <n> --merge`(**merge commit,不是 squash**;不需人類 approve)→ **land 驗收兩層**:①`git merge-base --is-ancestor <sha> origin/main`(**祖先判定,不是相等**——走 merge commit 之後 main 的 HEAD 必然不等於你的 sha,用相等判定會誤判成沒 land)②**另外直接讀 main 樹上的位元組**確認改動真的在(`git show origin/main:<path>`),不要只信 ① → autodeploy 盯 `/api/version` git_sha(公開免 auth;version 欄恆 0.0.0,對賬看 git_sha)→ 收退路(`worktree remove` + `branch -D`)。⚠️ **這裡曾寫「FF fast-forward push(`git push origin <b>:main`)」,已被現況推翻**:主幹現在有分支保護,直接推會被拒;owner 2026-07-31 裁定**以現況為準**,走 PR。

...
13. **land / verify 紀律(AI-friendly:宣告前先查證)**:
    ...
    - **land pipeline(走 PR,不是直推 `main`)**:off `origin/main` 開分支 → 隔離 worktree、標死 scope → 親驗全 diff(無 scope-creep)+ 親讀關鍵邏輯碼 → `bash bin/ci.sh` **rc == 0 且最後一行**精確為 `[ci] all green` → push **分支**(`git push origin <b>`)→ **讀回確認**(`git ls-remote origin <b>`——push 沒報錯不等於推上去了)→ `gh pr create` 開 PR → 等 PR 上的 `cloud-gates` check 綠(`.github/workflows/ci.yml` 的唯一 job,`pull_request` 觸發,跑 `bin/ci-cloud.sh`;它是乾淨 Linux 的 cross-check,**land 權威仍是本機 `bin/ci.sh`**)→ **合併前先 rebase 到最新 `origin/main`**(`git rebase origin/main` + `git push --force-with-lease`,並重跑 `bin/ci.sh` 到綠——rebase 後的樹沒被驗過)→ 合併 PR(`gh pr merge`)→ 雙源查證(`git ls-remote origin main` + `gh api repos/pkyosx/OffiCraft/commits/main --jq .sha`)→ autodeploy 盯 `/api/version` git_sha(公開免 auth;version 欄恆 0.0.0,對賬看 git_sha)→ 收退路(`worktree remove` + `branch -D`)。
      ⚠️ **`main` 是受保護分支,直推(`git push origin <b>:main`)會被 `protected branch hook declined` 擋掉**——本條在 T-6d74 從「FF fast-forward push 進 main」改寫成 PR 流程,就是因為舊文寫的那個動作照做必定失敗,而實際 land 一直走 PR(`git log --merges` 上的 `Merge pull request #NN` 就是證據)。**保護是要繞過的對象、不是要放寬的旋鈕**:不要 `--admin`、不要改保護設定,也不要為了讓某道 check 過而放寬它。舊文用 FF push 換來的「主幹上沒有未經驗證的意外交錯」,現在由上面那個 **rebase-to-latest-main + 重跑 CI** 的步驟承接;合併本身在 GitHub 端產生 merge commit(現況即如此),不強求線性歷史。

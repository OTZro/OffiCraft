...
path = Path(sys.argv[1])
text = path.read_text(encoding="utf-8")
needle = "讀取目前執行中的 build identity："
...
text = text.replace(needle, "MUTANT 讀取目前執行中的 build identity：")
...
if ! grep -q 'MUTANT 讀取目前執行中的 build identity：' "$WORK/authority.diff"; then
...
path.write_text(text.replace(needle, "MUTANT 讀取目前執行中的 build identity："), encoding="utf-8")
PY
...
if ! grep -q 'MUTANT 讀取目前執行中的 build identity：' "$WORK/drift.diff"; then

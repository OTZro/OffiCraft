...
- **發佈簽章 = 觀測不 enforce(T-33d5)**:發佈機以穩定 self-signed 憑證簽 bindist 的 ocwarden/ocagent(`bin/codesign-artifact`,讓 TCC 授權跨版延續);self-update swap 後只 **log** 新 binary 的簽章身分(`signatureOf` seam → `codesignIdentity`),**絕不驗簽硬擋**——self-signed 憑證在未信任機器上 verify 必非零,硬擋會 brick fleet。簽章只活在發佈 artifact，不進 git。詳見 `docs/dev/README.md` 發佈簽章節。
...
- 單一判定點 `validateTeardownTarget`(`teardown.go`),在 `resolveTeardownPaths` 推導出任何路徑**之前**跑。
...
- ⚠️ **任何驅動 `realMain(..."teardown"...)` 的測試一律要注入 `teardownSysOps` 假貨**(`namespace_test.go` `withFakeTeardownSys`)。realMain 的 launchd 目標來自**真實 `os.Getuid()` + canonical label**——假 HOME 只隔離**檔案路徑**、不隔離 launchd。2026-07-26 這個缺口在 review 期間兩度 bootout 掉本機 live warden。守衛本身不是測試的防護,seam 才是。

...
- (已拆除)`cli/ocrelease/` 與 `server/ocupdaterd/` 隨 t-dc68 退役:發佈改走 GitHub Releases(`bin/release publish --beta <tag> --target <sha>` 打包並出貨、beta→final 子命令翻正式版;⚠️ **`bin/release <tag>` 與「印一行 `gh release create` 給人貼」的半套形式都已在 T-588c 移除**,打舊形式會拿到非零退出 + 正確替代指令,詳見 `docs/dev/README.md`〈發版指令〉;server 端 update_check.go/upgrade.go 直接對 GitHub API 檢查與升級,見 `server/CLAUDE.md`)。

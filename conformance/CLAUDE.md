...
- manifest 的 row set 由 Go 側的 `TestEveryServedRouteIsInThePermissionManifest`（T-61）釘回 `routes.go` 的 route table，兩個方向都會紅並指名；本目錄的檢查全部從 manifest 出發，**構造上看不到 manifest 沒學到的 route**。

...
`test_sse.py` + `sse_client.py`:活 SSE client(httpx stream + 背景 thread + queue,純黑箱)。timeout 紀律:**每個等待都先用 HTTP write 觸發事件**(delta 在一個 0.25s poll 內到),絕不空等 15s heartbeat;negative 等待 1–1.5s 限量使用。覆蓋:§1 gate/headers/`: connected`、§2 delta frame 形狀(id==seq==epoch、五鍵 envelope、remove ⇒ deleted+payload:null)、§2.1 seq 嚴格遞增=發佈序、§3 **閉集的每一個 topic 全數觸發觀測**(op patch/remove/signal)——⚠️ **閉集大小刻意不寫死在這裡**(那個數字每加一個 topic 就過期一次,而 `triggers` 表原本正是一份手抄的 9 條清單,閉集長到 12 之後 `task`/`outsource_worker`/`task_manual` 的寫入面整段刪掉全套件照樣全綠)。現在 `test_every_closed_topic_emits` **在執行當下讀 `spec/sse.md` §3.1 那張表**(產品側的 wire 契約、也是 `hub.go` 的 `sseTopics` 自述的來源)並與 `triggers` 表的 key 集合**對質**:少了誰、多了誰都會被點名。**不要在測試裡再抄第三份清單**——那只是把同一個缺陷換個地方。⚠️ 讀 §3.1 的 parser **fail-loud、沒有 fallback**(`_parse_closed_topics`:heading 找不到／段落切不出／解析出零個 topic 一律 raise `SpecTopicParseError`)——舊版用 `str.split`,而 **Python 的 `str.split` 找不到分隔字串會回傳原字串**,heading 一被改名就默默改成切另一段,而那段**照樣解析出健康的答案**,守衛整支靜默失效(實測 old parser 對「拿掉 `### 3.1`」的文本仍回同一個集合)。🔴 **這裡曾經把原因寫成「退化成解析整份文件,又剛好因為 §4.1 audience 表列著同樣那 12 個 topic」——那個因果是錯的**(獨立審查複驗 + 我自己純函式重算):退化後**不是整份文件**(掉 `### 3.1` 是「檔頭→§3.2」的約 **9.9k** 字元切片,全檔 34207);**與 §4.1 無關**(§4.1 在 char **11089**,切片根本沒讀到,而且 §4.1 只列 **7** 個 topic,湊不出那個集合);**把 §4.1 整段刪掉,退化答案一模一樣**。**真正的機制更普遍也更危險:結束界 `### 3.2` 位在目標表「之後」,所以起始界一失蹤,退化切片仍然完整包含它本來要框住的那張表**——答案對是因為表還在切片裡,不是因為別處有第二份表。⇒ **任何「用位於目標之後的 heading 當結束界」的 split 型 parser,起始界一失蹤就會靜默給出正確答案。**(這條有行動意義:相信 §4.1 版本的人會以為「清掉 §4.1 就解除風險」,實測不會。)`test_spec_topic_parser_fails_loud` 用構造輸入釘住這三種降級,且**斷言 mutation 真的移除了分隔字串**(把 `### 3.1` 改成 `### 3.1bis` 仍**含有** `### 3.1`,那樣的 fail-closed 測試是恆真的)。⚠️ 另一個易誤修的點:`context-high`(§6)／`warden-command`(§7)／`task-close`(§8)走 `PushDirected`、**不經 `Publish`**,所以**不在**閉集裡也不在 `sseTopics` 裡——那是刻意的獨立 envelope family,不是漏掉,別「補」進去;它們由各自的測試釘。⚠️ **`member` 那一列釘 value binding(主角),迴圈前另有一道 barrier(配角)**:`wait_for_frame` 是從佇列前端 drain,會吃到**連線開啟後、迴圈開始前**的 setup 寫入所產生的舊 frame,`member` 那一列曾因此**恆真**(publish seam 整個拿掉照樣全綠;實測重現)。**主要守衛是 value binding**——斷言收到的 frame 帶著**這一次 PATCH 剛寫進去的值**(payload 是 `hub.Publish` 當下的 eager snapshot,更早發出的 frame 不可能帶到新值),所以那一列**不需要任何 mutant 就能自證非恆真**;實測:把 barrier 拿掉(其他不動)那一列立刻紅,payload 是 setup 的 `conf-scratch-*`。🔴 **這裡曾經寫成「identity binding:舊 frame 的主體天生不是這一個」,那句是錯的**(獨立審查 round 2 實測):污染源 `fresh_member()` 回傳的**就是**後面 PATCH 的那個 member,`payload["id"] == member` 對舊 frame **為真**,主體完全沒有鑑別力,承重的只有值。⚠️ **退化條件**:這條守衛只在「那一列 PATCH 的欄位、其值會被 payload 回顯」時有效;哪天改成 PATCH 別的欄位就會退化回恆真——所以 PATCH body 寫成具名 dict,並先斷言 `"name" in member_patch_body`,**改了沒重綁就會紅**,不會靜默退化。**配角是 `sse_client.drain_backlog`**:吸收既有 backlog、**等到**串流靜默才回傳(外層總 deadline 才是紅),它保護的是另外 11 列沒有值可綁的 row,並讓合法 setup 寫入不會把那一列變成假紅。🔴 **那 11 列的安全是「借來的」,不是證明過的**——它們仍然只斷言「某個該 topic 的 frame 到了」;而且實測(審查 round 2 全 frame 追蹤)**這個 codebase 裡「一個觸發發多個 topic」是真的**(建回覆卡也發 `chat`、建角色也發 `member`),今天沒害到任何一列**純粹是因為那些 cross-talk 的 topic 在 triggers 表裡排在前面、frame 已被消費掉**。⇒ **這是順序依賴,不是性質:重排 triggers 表就會有某一列靜默變恆真,而且沒有任何東西會抗議。** 要重排或新增有 cross-talk 的觸發,就照 `member` 那一列把該列綁到自己的寫入值。⚠️ **兩個一定要照抄的教訓**:(1) 這道 barrier 的 v1 寫成「**斷言此刻靜默**」,結果 baseline 直接紅——setup 寫入的 frame 走 0.25s fan poll、**本來就會晚到**;**晚到不是錯,沒被吞掉才是錯**,所以正解是「**等到靜默**」(看到 delta 就重置靜默窗),不是斷言。(2) **任何吸收型 barrier 都必然看不到自己回傳之後才發生的寫入**——在 barrier 之後插一筆 setup 寫入,那一列會是**綠**,而且是恆真的那種綠;不要設計那個實驗再把綠讀成安心(實測:value binding 在 → 紅;把它拿掉 → 全綠)。也不要退化成「把已知的那幾次 setup 寫入搬到連線之前」——那只擋今天這幾筆、下一個人多加一筆就靜默重新污染、§5 presence 純看連線(offline→online→offline)、§5.2 first-connect 清 waking / last-disconnect bank cost **恰一次**(pop 防 double-bank)、§5.1 dual-SSE takeover(第二條連線 200 接手、舊 stream 被 server 終結、presence 不閃斷)+ 防震盪 throttle 超額 409 conflict envelope(pre-stream)+ owner 連線豁免、§6 context-high warn 定向帶(無 id: 行、只到 agent 連線、stale-pct guard)、§7 warden-command band(真 onboard warden token 收 START frame、args 全形狀、member_token 可登入、絕不漏到 owner fan-out)。

test_sse.py 另含 **zombie stop gate 系列**(殭屍防線 B):stop 生效中(deactivate 後)重連 → pre-stream 409 conflict envelope 且不算 online、activate 原子解 gate、剛 hire 未 activate 照常連(下界)、dismissed member 拒連、warden 豁免 desired-offline 臂。

`test_lifecycle.py`:spec/lifecycle.md。§1.1 claim envelope(header 恰 HS256/JWT、machine_id 缺席=整鍵省略)、偽造 token 的防護(篡改簽章/alg:none/過期 payload → 401)、§1.3 mint TTL(login 86400、mint ttl_days 與 400 天 cap、onboard 90 天、bootstrap 帶 desired_machine_id claim)、§2 boot fold **byte-for-byte 重建**(seed 檔為語言中立資產直讀;觀測 wire 對 seed 做 `{OWNER_ID}`→`owner` 代換,spec 未載已回報)+ 空 user block 整段跳過 + lessons overlay-wins + unknown role 404、in-memory #2 observed position(agents_on_machine 擋 machine uninstall **與 delete** 409——實際線上判準:斷線即放行,離線綁定不擋)、§4.3 uninstall 一次性意圖(owner 定案 2026-07-11:warden 斷線 → 意圖消化歸零 offline;boot-command 重取等重裝路徑先歸零殘留意圖再裝)。**DEGRADED 誠實表**(附理由、有 teeth):restart honest-empty / seq rollback(黑箱不可重啟 target)、15s heartbeat(單次觀測費 15s)、多 owner fan-out(單租戶無反例)、reconcile timers(分鐘級)、secret tier、boot_ts 直接值(由 stale-guard 間接)、queue cap/at-most-once 掉幀。

## 回覆卡面(M2 reply-card B1)

`test_reply_cards.py`:等我回覆卡的狀態機與 pane 語意。開卡=waiting 且同時騎聊天流(chat 訊息 meta.reply_card_id ↔ 卡 chat_message_id 雙向連結)、**只有回覆能關卡**(選項/打字反問/附件皆算;無 close/skip 路由=by construction,405/404 探測)、一張卡一次性(二次 POST answer 409)、重新決定僅限已回應卡(PUT on waiting 409;換答案不變態、不回徽章)、開卡驗證(kind 閉集、options 1..4 非空白、summary 非空)、待回覆 pane 等最久在前、近期已回覆 pane 新在前、**list 走輕量摘要(T-3f31 owner 定案:卡只需要 title+決策)**——每列只有 summary+狀態+決策 digest(option_idx+選項原文、answer text 截 200 rune 預覽、附件為 COUNT),**不帶 body/options 全文/chat_message_id**(全卡走 get_reply_card;`test_list_rows_are_light_title_plus_decision_only`),`?limit=N` 在 pane 排序後截前 N(0/負數=全量;`test_list_limit_caps_rows_after_pane_ordering`)、**`?view=full` 回同一個 pane 的全卡(T-a3e4)**——`test_view_full_serves_the_same_pane_as_whole_cards` 逐列斷言 `row == get_reply_card(row.id)`(所以那條同時釘住「client 不會拿到第三種形狀」)+ 順序與 `?limit=` 不變 + 同一張卡的 light digest 仍是 COUNT 而 full 是 refs(兩種形狀在**型別**上就不同,不是長短之別);`test_view_defaults_to_light_and_rejects_anything_else` 釘**預設不變**(不帶參數 ≡ `?view=light`,逐位元組)與未知值 400 點名兩值 + 陽性對照;`test_view_is_not_advertised_to_agents` 對**線上** tools/list 斷言 `list_reply_cards` 的 inputSchema **沒有** `view`(而 `status`/`limit` 有——反恆真:證明是「刻意不給」而非「這個工具什麼都不廣告」)。⚠️ **那條刻意打線上而不是比對凍結檔**:agent 看得到的是 server 真的服務的東西。徽章只算 waiting、**答案帶脈絡回 agent**(agent 自己的 SSE 連線收 reply_card delta → get_reply_card 拿回 summary/選項原文/owner 答案+附件 round-trip)。DEGRADED(誠實):近期已回覆的 24h 過期是時間性、黑箱不可觀測——窗界值由 server 單元測試(api_replycards_test.go)釘,本檔釘窗的成員資格+排序。routes 面:6 條新路由進 auth 矩陣(answer 兩條 requires=owner)與 happy 表。**已過期終態(T-1aa4)**:`POST /api/reply-cards/{card_id}/expire`(requires=owner、MCPExclude)= 唯一另一個出口——不是回答(200 帶 expired_ts、無 answer digest)、終態(double-expire/answer/PUT on expired 全 409、expire on answered 409)、徽章 -1、`?status=expired` pane(24h keyed expired_ts)+ count.expired、gate 卡 expire → step/task 退回 in_progress(答卡 resume 的 expire 攣生)、**orphan 卡(task 終態、answer 409)唯一出口 = expire 且關閉任務不動**、expired delta 走同一 reply_card downlink(payload status="expired")。矩陣/happy 各 +1 列(positive face 每次燒一張 fresh waiting 卡——expire 一打即終態)。

## 任務面(M3 任務批 Phase 1)

`test_tasks.py`:M3 任務系統的狀態機、gate↔回覆卡打通與手冊生命週期。全迴圈(建 ad-hoc 任務→in_progress→交計劃(含 gate 預告 = is_gate ∧ 無卡)→推節點→open_gate 生效(真 M2 卡:騎聊天流、待回覆 pane 帶 task 物件、step 綁 reply_card_id)→owner 走**既有** answer 路由答卡→**server 自動把 step 與任務還原 in_progress(T-68b7:waiting_owner 完全由卡片生命週期夾住;僅解除 hold、絕不會替你推進節點——H4「不會替你推進工作進度」核心保留,其「答卡後停在 waiting」語意由 T-68b7 取代)**→agent 自行推進→done(closed_ts、progress 3/3、徽章 -1))、去重(開放同鍵 → 200 + deduped:true 回舊任務;異鍵造新;終態撞鍵重開造新,H1/H2;缺必填 input → 400)、終態防呼(status/plan/deps/step/gate/二次 terminate 全 409;steps 原樣凍結)、狀態機 wire 守衛(waiting_owner/terminated 不可直設 409、未知值 400、非法轉換 409、waiting_external 必帶原因 422 且離開清空)、deps 純標示(自指/未知 422、status 不動、全量替換)、任務訊息框(owner→executor 普通 chat + meta task 脈絡、空訊息 400)、手冊 CRUD(部分編輯、agent learnings whole-doc replace、撞鍵 409、刪除防呼:有未結案任務 409 → 關閉後可刪 → 讀 404)、get_my_task 成員 404 / warden 403(agent 級 floor 首用)、**T-1aea replan 保留已答卡節點**(答卡後 replan → 該節點凍結 `superseded` 終態(原位、finished_ts、卡 join 留存)、waiting 卡節點照舊被替換、superseded 不計 progress、報離/重綁 gate 皆 409)。**DEGRADED(誠實)**:外包 worker 黑箱不可建立(Phase 2 排程器是唯一建立 seam)——領任務正面(assigned→active + 手冊快照)與解僱由 server 單元測試(api_tasks_test.go)釘。routes 面:21 條任務路由進 auth 矩陣(agent 級首用;執行者守衛 = agent_other 403 override)與 happy 表。**§6.2/§6.3 批**:close-out 回報(`POST /api/tasks/{id}/closeout`——開放中 409、done/terminated 皆可報、冪等重報 200 no-op、執行者守衛、`closeout_reported` 旗標)與 resume-summary 任務段(caller 為 executor 的非終態任務;**T-3f31 輕量摘要**:每張只有 task_no/標題/type/狀態/優先權/當前節點 id+**名稱**/進度/`detail_chars`(省略掉的計畫文字 rune 數),**不帶 steps/DoD 全文**(細節 get_task 拉);另帶 `overview` size/概要塊(chat_count、tasks_returned/tasks_open_total(cap 外真實總數)/tasks_detail_chars、caller 自己的 cards_waiting/cards_answered_recent 隨答卡即時 fold、T-1b09 roster_chars/machines_chars)——peek-then-decide。⚠️ **T-1b09 的 `roster`／`machines` 兩塊目前 conformance 未覆蓋**(此處只釘 method/path/auth 與 tasks/overview 型別、不列欄位)，它們由 server 單元測試 `api_chat_resume_floor_test.go` 釘住;這行是覆蓋缺口的誠實標註,不是待辦;identity-lock、上限 5 張依 updated_ts 新→舊)一併釘在本檔。

## 錯誤 envelope 面(第二批)

`test_error_envelope.py`:全站錯誤唯一 wire 形狀 `{"error":{code,message}}`(docs/design/api-error-envelope.md)。代表性錯誤路徑逐類發射:401(無 token / 壞 token / 錯密碼)、403(below-floor 打 owner 路由)、404(語意 unknown member + 框架 unknown path 兩源)、405(框架 wrong method)、409(refocus offline)、400(handler 輸入拒絕:壞 base64 附件、非數字 context_pct、全空 telemetry——lifecycle.md §3 釘 flat 400 非 422;⚠️ **該條有 EXCEPTION**,T-90be 起 `hardware`/`claude`/`runtimes` 三個**已宣告形狀**的 block 傳非物件是 decoder 的 **422**,未宣告的 `binaries`/`rate_limits`/`tokens`/`command_result`/`self_update` 才仍是 flat 400;`{"hardware":null}` 與 `{}` 一樣走 all-absent 400,`{"hardware":{}}` 是 200)、422(RequestValidationError 源:login 缺欄、install.sh 缺 token、以及上述三個已宣告 block 的型別拒絕 = case `422-telemetry-declared-block-wrong-type`,打 `{"hardware":"not-an-object"}`——它與 `400-telemetry-empty` 是同一條路由的兩半,兩個拒絕由不同的源發出,都必須帶同一個 envelope)。斷言:body 恰好 `{error:{code,message}}` 兩鍵、code 符合閉集 `CODE_BY_STATUS`(側鏡重宣告、不 import)、message 非空、無 legacy `{"detail":…}` 殘留。

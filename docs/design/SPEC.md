...
### 全域情境（Global Context）
**套用到所有成員**的 boot context，拆成三塊（依組裝順序）：**系統互動**（系統運作說明）→ **使用者自訂**（additive 追加塊，空時整塊跳過）→ **啟動程序**（工作室固定 SOP）。於「設定 › 角色誌」檢視；**三塊都可編輯**。系統互動與啟動程序曾經是唯讀渲染、後端也真的沒有寫入端點（那正是「by construction」那句話的世界）；**T-791e 起兩者都有**：`POST /api/system-interaction`、`POST /api/boot-sequence/{runtime_key}` 與各自的 `/reset` —— **動詞是 POST，打 PUT 會拿到 405**。授權下限是 owner 或 admin 特助。這兩塊與下線程序由同一個 `BootDocPage` 呈現，共用同一套編輯與版本歷史；**還原出廠版不是頁面上的並列按鈕**，它在編輯模式裡的版本清單「初始版本」那一列（owner 2026-08-14 裁定）。出廠 seed 永遠不被覆蓋，還原拿得回原文。UI 一律**不顯示 .md 檔名**（三塊是內容、不是檔案）。
...
- 角色誌列表依 boot 組裝順序列三列（**不顯示 .md 檔名**）：
  - **系統互動**：點進 → `BootDocPage`：檢視 markdown；**編輯** → 編輯模式，模式內有版本歷史，**還原出廠版在版本清單的「初始版本」那一列**（不是頁面上的並列按鈕；出廠 seed 不被覆蓋，永遠拿得回來）
  - **使用者自訂**：點進 → 詳情頁：檢視 markdown；**編輯** → 編輯模式（**取消編輯／重置／完成編輯**）；**重置**＝清空（回到未撰寫的 `預設` 空塊；boot context 組裝時空塊整塊跳過）
  - **啟動程序**：點進 → **兩列的索引頁**（Claude／Codex 各一列，不是兩份文件疊在一頁）→ 再點一列才進 `BootDocPage`，各自可編輯與還原

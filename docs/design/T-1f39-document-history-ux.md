...
| 上限 | `contextDocMaxChars = 10000`（rune），`DocCapBlocked(before, after)`：未滿放行／超標但比原本短放行／超標且不短於原本擋下 | `server/ocserverd/domain.go` |
...
2. **量級撐得住**。受版控的長文有 10,000 rune 硬上限（角色誌、SOP、學習經驗），行數量級數百行，LCS 行級 diff 在瀏覽器是毫秒級。
...
> （T-ae38 起是四個：`doc_cap_chars_duty` / `_insight` / `_learning` / `_manual`，角色定義預設 1000、其餘 10000；
> 角色定義也是 T-ae38 才開始有上限的）。以下這段沿用單一 `doc_cap_chars`
> 設定（預設 10,000、範圍 10,000..100,000，只能往上調）。本節寫「10,000」的地方一律讀作**當下設定的上限**；
...
理由是可讀性的量級問題——受版控的長文上限一萬字，三版同時攤在清單上，等於要滾過兩份不相干的長文才走得到想看的那一版。

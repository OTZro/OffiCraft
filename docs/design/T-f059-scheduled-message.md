...
> `_first_existing_instant`）採同一條規則：每次 +1 分鐘往後找。**只有上限不同。**

#### 🔴 往後找的上限：**掃到那個日曆日結束**，不是猜一個分鐘數

初版寫的是「上限 120 分鐘，因為 tz 資料庫裡的 gap 從來不超過一小時」。
**那句話寫下來的時候就是假的。** 掃 tzdata 2025b 全部 **599** 個時區、2015–2040（我自己重跑過一次，數字對得上）：

| gap 大小 | 轉換次數 |
|---|---|
| 30 分 | 54 |
| 60 分 | 5,350 |
| **120 分** | **26**（`Antarctica/Troll`，其中 **14 次還在未來**，最遠到 2040） |
| **180 分** | **6**（`Antarctica/Casey`，2016–2022） |

- `Antarctica/Troll` 每年三月 **01:00 → 03:00**。一條 01:00 的排程需要往後挪**恰好 120 分**——
  舊上限剛好塞下、**餘裕是零**，而註解還寫著那是「headroom」。
- `Antarctica/Casey` 的 **180 分**（實測 **00:01 → 03:01**）**超出舊上限** ⇒ 那幾次是**靜靜被丟掉的**。

**所以上限改成「那個日曆日的結束」。** 理由：
1. **日界不會被下一版 tzdata 推翻，分鐘數會。** 這條註解已經因為猜錯而說過一次謊，不該再猜第二次。
2. **它讓兩種缺席不再共用同一個出口。** 「這個讀數往後挪到當天稍晚」＝那一次照常發生；
   「這一天從這個時刻起完全沒有讀數」＝**這個時區沒有這一天**（`Pacific/Apia` 1994／2011 直接刪掉一整天），
   於是往前一天再找。舊寫法把「gap 比我猜的大」跟「整天被刪掉」混在同一個出口，前者因此變成無聲漏送。
3. 順帶修掉一個舊的小滲漏：日終的排程遇上被刪掉的一天時，不會再挪到**隔一天**去。
4. 成本可忽略：正常日子第一次就命中，只有真的碰到 gap 才會走迴圈，最壞 1,440 次純算術。

⚠️ **真的一個讀數都找不到時的行為，現在是明講的**：`mostRecentSlot` 回 `ok=false`，
而 tick **每一輪留一行 log**（`skip … no slot exists in the lookback window — this schedule is NOT delivering`）。
這種情況實際上不該發生（daily 往回找 3 天、weekly 15 天、monthly 13 個月，正常時區不可能全部沒有讀數），
所以它是一個「不該出現」的訊號，**噪音是刻意的**：一條不再送出的排程必須吵。
在此之前這條路是**完全靜默**的，而那正是本票要消滅的形狀。

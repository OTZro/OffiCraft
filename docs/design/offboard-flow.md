...
**第一層（零複製規則本身）**：初稿在這裡指控 `docs/guide/architecture.md` 抄的那個秒數已經變成假話。**那個數字確實過期了**：那一段列了換手的三種觸發，而**你按 Refocus** 那一種等的是 `soft_offboard_grace + recycle_grace`，不是它寫的那個值（見第三節）。**抄下來的數字不會在它失效時報警**——這正是零複製規則要防的事，而它就在這份文件的鄰居身上發生了。
...
🔴 **而那句話一共有五份拷貝，不是一份**，掃出來才知道——`spec/lifecycle.md`、`docs/guide/architecture.md`、`server/CLAUDE.md`、`spec/openapi.json` 的強制下線端點說明，以及 `api_members.go` 裡那個 handler 自己的註解。其中兩份特別要緊：`server/CLAUDE.md` 是**每個實作者真的會讀到的**，而 openapi 那份是**唯一那顆按鈕自己的說明**——它原本寫著「繞過 120 秒的寬限計時器」，等於告訴讀者「不按也會有人收」，**而那正是這條裁定唯一不能被誤解的地方**。⇒ **盤點一句假話時，先問它有幾份拷貝，再問哪一份的讀者會照它行動。**）

...
這條規則有代價經驗撐著，而**這份文件自己就是那個代價**：初稿在這裡指控 `docs/guide/architecture.md` 抄了一個具體秒數、而那句話已經變成假話。**那個指控是錯的**——那一段的標題是「什麼時候會換手」，整段講的是**換手**，而換手真的有那個窗（見第三節的三路對照）。**我把一份講換手的文字讀成講下線，然後宣布它是假的。**

⇒ 真正的代價經驗因此換成一條更難防的：**抄一個數字會過期，而「引用一段文字時把它的脈絡讀錯」連過期都不必，它一開始就是錯的**，並且看起來跟一個查證過的指控一模一樣。抄下來的數字不會在它失效時報警，**讀錯脈絡的指控更不會**——它引用了真實存在的句子，只是安在錯誤的那條路上。
...
（`spec/lifecycle.md` §4.3 原本仍描述舊的「arm 一個 deadline、逾時自動送 stop」行為；owner 2026-08-17 授權補齊，已跟上。同一句話的第三份拷貝在 `server/CLAUDE.md` 也一併修掉了——**那份才是每個實作者真的會讀到的**。）
...
## 附：這份文件的錯誤清單（五輪審查抓到的十七項，＋一項 land 之後才碎的）

本文經過一次逐句事實核對加四輪獨立審查。**每一輪都有新的錯，而且後面幾輪的錯全部出在「為了修上一輪而新寫的句子」裡。** 最後一項（第十八項）不屬於這個形狀：它**通過了全部五輪、也通過了 land**，是後來有人拿這份文件去執行時才碎的。
...
### 第十八項：land 之後、由「去執行它」的那個人抓到的

前面十七項都是審查在 land 之前抓到的。**這一項不同：它撐過了五輪審查、撐過了 land，是後來有人拿著這份文件去做事時才碎的。**

本文第一節原本指控 `docs/guide/architecture.md` 抄了一個秒數、那句話已經變成假話。後來 owner 授權去補那些落後的文件，**執行的人打開那一段一看——它的標題是「什麼時候會換手」，整段講的是換手，而換手真的有那個窗。那句話從來沒有假過。**

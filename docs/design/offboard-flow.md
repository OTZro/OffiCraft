...
| **server（reconcile 迴圈）** | 比對「想要的狀態」與「觀察到的狀態」，決定該送出什麼命令 | **持有不能丟的記憶**。它有 in-memory 狀態（上一道命令、重試窗、失敗退避與斷路器、殭屍二次確認窗），但那是**可丟的**——server 一重啟就從當下觀測重新推導 |
...
⚠️ 「可丟」不等於「丟了沒差」：那一丟同時把**失敗退避與斷路器**歸零（一個反覆起不來的 member 會拿到全新的重試預算），也把殭屍的二次確認窗重新開始算。兩者都往安全的方向倒，但**重啟後的第一輪跟穩定狀態下的一輪不是同一件事**。
...
它的效果就是表格裡那三件：換座艙燈號、打一個時間錨、廣播一則 delta。**那個時間錨後來被「強制下線」的判定拿去當比較基準**（用來分辨「這一次切斷發生在你開始收尾之後」）——那是我查到的、它超出顯示層的用途；我沒有窮舉過所有讀這個欄位的地方，所以不寫成「唯一」。

⚠️ 本文初稿在這裡連錯兩次，兩次形狀不同，都留著當提醒：先寫「時鐘照走」（下線那條根本沒有時鐘可走），被打回後改寫成「它是 SSE 重連被放行、以及自請重啟被接受的依據」——**那個替代理由兩半都假**。打不打它都連得上；而自請重啟看的是「有沒有 live 連線」加「意圖仍是上線」，碼裡的註解甚至直說**這個宣告曾經是自請重啟被誤拒的原因**，門檻改用連線判定就是為了修掉它。

⇒ **被打回之後當場補上的理由，跟原本那個一樣需要被查證。** 修正不會因為它是修正就自動為真。
...
⚠️ 這是**殺路徑專用**的解析，不是所有命令的通則：其他命令一律送往指定的那台。
...
**fail-closed 講的是「不確定就別動手」，不是「不確定就別出聲」。** 上面每一格在不動手的同時都會留下痕跡：兩格寫進那一列 member row（座艙讀得到），三格寫進 server 的 log。

...
正職成員走 reconcile 那個產生器。**外包 worker 走的是第二個**——它在碼上自述為 reconcile 的**同構雙胞胎**：一樣有一個純決策核心、一樣有週期 tick、一樣有事件驅動的立即 tick、**一樣有自己的關閉開關**。worker 的 START 完全不經過正職那個決策器。

⇒ 所以要說「決策與執行分離」是對的；但說「**只有一個決策者**」是錯的。**有兩個。**

### 🔴 上線這條**有**繞道，只是不在正職那邊

下線有「強制下線」那種繞過決策器的直接投遞。上線這邊的對稱物是**外包的 owner op**：換機器、重生那幾個動作會**立刻直接投遞**，碼上的註解逐字寫「立即，而不是延到下一個 tick」。

⇒ **正職沒有繞道；worker 有。** 初稿寫成「上線這條沒有等價的繞道」，是把正職的形狀當成了全部。
...
worker 那邊另外還有兩種：**排程把一張外包任務收進來時的 admission**，以及 **owner op 的立即重生**（換機器、重新聚焦之後）。
...
1. **明確 pin 是 hard 的**——落空就停在那裡，**不改派**。這一層才是 owner 2026-07-25 裁定砍掉的東西：**「pin 落空時自動改派」被移除了。**
2. **上一次實際跑在哪台是 soft 的**——那台不能派時**會往下掉**，碼上的註解自己聲明「這不是被禁掉的那種自動放置」。
3. 第三層是**排程用的可用機器選擇**。

⇒ 而且 worker **開機失敗會把那台機器 bench 掉並冷卻**，下一輪就落到第三層。**所以「外包被擋下之後會原地站著不動」只有在 pin 那一層成立**，初稿把它寫成了全稱句。
...
⚠️ **那個「閉集合」在碼上已經漂了**：宣告它的那段註解只列了七個理由，而實際會發出的比這個多。要引用這個集合的人，**去讀真的會發出的那些，不要讀那段註解**。

### warden 不重試——但這句話的射程要收窄

**不重試的是**：START 這道命令本身、檔案寫入、CLI 連結、回執的回傳。**這些失敗就是失敗，重試是 server 的事**——兩邊都重試會變成同一個動作被做了兩次而沒人知道總共幾次。
...
**清掉這個錨的地方有四個**，不是兩個：連線第一次接上、等太久放棄時、**按上線那一下自己就會先清一次**、以及首次開機的收尾。
...
投遞成功只代表**排進了一個佇列**；取出是**破壞性的**，取出後寫失敗的那一份**誰手上都沒有**，而 **START 是直接丟掉不重排的**（它帶著一張當下的 token，留著也沒用；**唯一會被重排的是另一種命令**）。「寫進了通道」**不是 ack**。
...
2. 🔴 **重啟後的第一輪，不等於穩定狀態的一輪。** 產生器的記憶是可丟的，一丟**退避就歸零** ⇒ 一個反覆起不來的成員會拿到**全新的重試預算**。**那是更積極，不是更安全。**（同一個形狀在下線那條也有，`offboard-flow.md` 記過。）而殭屍二次確認窗重開則是**安全方向**——同一次重啟，兩個相反的倒向。
...
初稿附錄留了四格「刻意沒有回答的問題」，並且在任務紀錄裡把它當成誠實的表現。**事實核對一查，其中三格根本有答案**，第四格則可以講得比我更確定：

- 座艙怎麼呈現喚醒中 —— **有專屬的前端規則檔**，而且我正文裡早就斷言了「那才是 owner 真的看到的」⇒ **我一邊說不知道，一邊拿它當論據。**
- 受理由的覆寫順序 —— **碼上有明文的優先序**，而且方向跟我正文寫的相反（見第九節）。
- env 是白名單還黑名單 —— **那是假衝突**：兩份文字講的是**兩個不同的接縫**（server 起 warden 子行程 vs warden 給 agent 的環境），一個是白名單、一個是加法 env 檔只擋保留字首。**我看到兩句話矛盾就停手，沒去問它們是不是在講同一件事。**
- 斷路器有沒有呼叫者 —— 碼上的註解**逐字**寫著沒有。

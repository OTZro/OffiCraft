...
（agent 拿到的第一段提示**不是**開機程序本身，只是一張**指路條**：你的身分與程序都在某個檔案裡，去讀它、照著走。真正的程序是 server 折給它的那份，而**啟動步驟那一塊放在最後**——**而且「放最後是為了壓過前面」就寫在碼上**：組裝那份清單的註解把它逐字標成「**recency-authoritative tail**」。⚠️ 這一句我上一版**把它降級成「我的解讀」**，那是被打回「講太滿」之後的過度修正——**把碼上的事實說成自己的猜測，跟把猜測說成事實一樣是失真**。）

...
- **warden 起 agent** 時給的環境：以互動 shell 的環境為底、疊上一份 env 檔，**只擋掉保留字首**（warden 自己的身分命名空間）與非法的 key。**它既不是白名單也不是黑名單，是加法**，而且失敗時傾向放行。
...
（agent 拿到的第一段提示**不是**開機程序本身，只是一張**指路條**：你的身分與程序都在某個檔案裡，去讀它、照著走。真正的程序是 server 折給它的那份，而**啟動程序那一塊放在最後**（組裝順序在碼上是規範的；「**放最後是為了讓它壓過前面**」**是我的解讀**，不是碼上寫的）。）
...
| 誰用它 | presence 推導（判它是不是「喚醒中」）、以及放棄的死線 | 換手提醒的一次性認領、回溯摘要 |
...
（本文目前**沒有**任何一格「不知道」。不是因為全都查清楚了，而是因為原本那四格都不成立——**如果你查到了新的界線，請寫進來，但照上面那個判準寫。**）

...
- 🔴 **`publish` 在 build 之前會先讀「這顆 commit 自己那一輪 GitHub Actions 的判決」,不綠就不發(T-b65e 立閘;T-7e6c 換證據來源,owner 卡 `rc-56fecb4d5aa3`)**。它**不再自己重跑一輪 CI**,而是拿 `GITHUB_RUN_ID` 去問 Actions API 兩件事:
  ...
  - **不帶 token**:repo 是 public,兩個 endpoint 匿名實測都 200;而 `auto-beta` 宣告了 `permissions: contents: write`,job-level permissions 是**整組覆寫**,`actions`/`checks` 因此都是 `none` —— 匿名反而是唯一穩的,也跟這份 workflow 自己的哲學(「gate cells use no token at all」)一致。

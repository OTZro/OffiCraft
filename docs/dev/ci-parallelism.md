...
可能假紅，也可能**在一棵沒驗過的樹上變綠**。後者是偽造 land 權威，所以直接拒絕。
...
🔴 **這一格從 2026-08-02（T-b65e）起不再只是理論**：`bin/release publish` 現在會在它切出來的
**staging worktree 裡跑一次完整 `bin/ci.sh`**（發版前的行為驗證閘）。所以只要有人在主副本跑
`bin/ci.sh` 的同時發版，就**恰好**構成上面這個未量測的情境——鎖不會擋（那是已驗的部分：兩棵樹
兩把鎖），能不能跑完則沒人量過。失效方向是**大聲的**（CI 紅 ⇒ 發不出去），不是靜默照發，所以
它不會偽造綠燈；但真的要並跑之前，還是先量這一格。

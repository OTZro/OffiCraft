...
> 開機還有一個過渡態：成員一起手會先報一次 **waking（喚醒中）**，等它掛上 SSE 連線才算 **online**。
...
這一步有寬限，**長短看是上面哪一種觸發**：**context 自動換手**與**成員自請 restart** 約 **120 秒**；**你按 Refocus** 那種
給的長得多——先是約 **10 分鐘**不催不趕的窗，之後才進最後 120 秒的收線（合計約 12 分鐘），因為那是你在等它把記憶
壓乾淨換一手，急不得。逾時沒收完，server 就**下令強制回收**（warden 執行）——寧可硬收，也不
...
報一次 **waking（喚醒中）**，等它掛上那條 SSE 長連線（`listen`）就算 **online**——這時它就正式接棒了。

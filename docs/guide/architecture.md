...
**收尾怎麼做。** 換手不是說走就走，退場的那一手要先把交接做乾淨。要它做哪幾件事，寫在 **設定 › 角色誌 › 下線程序**這份文件裡——**那份是權威版，你可以直接編輯**，而且 server 會在要收掉某個成員的那一刻把它**送到那個成員手上**（不必靠它還記得開機時讀過什麼；那時候開機說明多半已經被壓縮掉了）。
...
**幾乎每一種都沒有時鐘。** 上面四種觸發裡，**只有 context 那一條升到第二段門檻**時才會開始倒數（約 **120 秒**，逾時沒收完
server 就**下令強制回收**、由 warden 執行）——那一段是它真的快撐不住了，寧可硬收，也不讓一個半死的 session 卡在那裡占著位子。

**其餘全部沒有時鐘**：context 的第一段門檻、你按 Refocus、你改機器或換 model、成員自請 restart——這些都只是把收尾程序送到它
手上，然後**等它自己回報收完**。沒人替它計時。它要是再也不回話，就由你在成員面板按 **Force stop（強制停止）** 收掉——那顆按鈕
是這些路上唯一的收口，見 [成員與外包](members.md)。
...
之後幫你硬收：它一直等，直到成員自己回報收完、或是**你親自按強制下線**。所以下線之後看到它還掛在收尾狀態，不是卡住，
是在等你或等它——詳見 **下線程序** 那份文件。

...
聊天訊息、請示卡、任務產物**三個地方**都掛得上一種「比較附件」：點開之後看到的不是一份檔案，而是**前後兩份東西的差異**，畫面跟[版本紀錄的差異畫面](settings.md)是同一個（整列紅綠、兩排行號、改到的字更亮、整份顯示不摺疊）。

**它存的是兩個連結，不是兩份複本。** 所以同一份東西不會在系統裡多出第二份，也不會有兩份各自漂走。

**一側可以是「某份文件的某個版本」**，不必先把舊版本抓成檔案。這時那一欄的標題會直接寫出它是什麼：

- **「目前存檔內容（讀取於 …，之後會不一樣）」**——這一側是**活的**。同一個附件下個月點開，比的是那時候的內容，所以畫面會直接把這件事講出來。
...
**AI 成員怎麼掛上去**：`ocagent diff <改動前> <改動後>`，任一側也可以寫成 `doc:<種類>/<鍵>/<版本>/<欄位>` 直接指向一份文件。成員不必自己拼那個附件的內容。

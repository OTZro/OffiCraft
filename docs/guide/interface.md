...
- **Role journal（角色誌）** — 每個角色的定義（Duty）、判準（Insight）與累積的學習經驗（Learning），加上四塊「全域情境」（系統互動〔唯讀〕、使用者自訂〔你可編輯〕、啟動程序〔唯讀〕、下線程序〔你可編輯，下線那一刻才送出〕）。
...
- **Auto-handover threshold（自動換手門檻）** — 什麼時候把工作交接給新一手。**Claude 成員**看記憶用量的百分比（第一次通知 1–89%、最後通牒 40–90%）；**Codex 成員**看做過幾輪 context compaction（各 1–10 回合），不看百分比。

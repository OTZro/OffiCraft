...
- **Scheduled messages（定期訊息）**〔正職面板 / 外包詳情〕— 到了時間就把一則**預先寫好的訊息**送給這位成員的排程：名稱、訊息內容、頻率（每天／每週／每月）、時刻與**時區**（IANA 名稱，例 `Asia/Taipei`），以及一個啟用／停用開關（停用只是不再送，**刪除**才是永久移除）。訊息就是一則普通聊天訊息——他在線上就即時收到，離線就等下次上線再讀。⚠️ **每月選 29／30／31 號時，沒有那一天的月份會整個跳過**（不會改送月底），所以選 31 號的排程二月永遠不會送；卡片上就寫著這句。**這一區塊沒有顯示「上一次送到哪一格」**，要確認有沒有送出，看跟他的聊天紀錄。

...
- **Scheduled messages（定期訊息）**〔正職面板 / 外包詳情〕— 到了時間就把一則**預先寫好的訊息**送給這位成員的排程：名稱、訊息內容、頻率（每天／每週／每月）、時刻與**時區**（IANA 名稱，例 `Asia/Taipei`），以及一個啟用／停用開關（停用只是不再送，**刪除**才是永久移除）。訊息就是一則普通聊天訊息——他在線上就即時收到，離線就等下次上線再讀。⚠️ **每月選 29／30／31 號時，沒有那一天的月份會整個跳過**（不會改送月底），所以選 31 號的排程二月永遠不會送；卡片上就寫著這句。⚠️ **夏令時間往前撥掉的那一小時則相反：不跳過，往後挪。** 例如選在 02:30、而那天當地 02:00 直接跳到 03:00，那一次就在 **03:00** 送出，**隔天回到 02:30**（不會一直飄）。兩者不同是刻意的——「那一天根本不存在」和「那一天少了一小時」不是同一件事，而少送一次是沒有任何提示的。每一列底下有一行**上次送出**（還沒送過就寫「尚未送出」），所以「這條排程有沒有活著」在卡片上看得到；要看送出的**內容**仍然是去翻跟他的聊天紀錄。

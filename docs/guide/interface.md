...
- **Role journal（角色誌）** — 每個角色的定義（Duty）、判準（Insight）與累積的學習經驗（Learning），加上**送給成員的那些文件**，依四組陳列：上線、下線、任務事件、以及只顯示不給改的那幾份。⚠️ 其中只有一部分真的接上了送出端，其餘的編輯今天還不會改變成員收到的字——見 [設定與參數](settings.md)。

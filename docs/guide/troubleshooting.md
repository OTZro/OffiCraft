...
⇒ 監控那邊沒問題的話，這一步就是先把 `codex` 登入起來再重試。**監控那邊真的有問題**（機器離線、warden 沒連上），那就先修那台機器，這一節的問題要等它通了才看得出來；**登入了還是同一個框**，才回頭走「最近操作」那條——**認時間，不認內容**：只有時間比你這次按確認還新的那一行才算數（那時它寫的才是這一次的事，例如機器根本沒被選、或選的那台不是活的）；時間比你按確認舊的那一行，寫什麼都不要跟著做。⚠️ **那一行的時間不一定會刷新**——同一個原因連續擋兩次，它不會被重寫，所以「沒有更新的一行」不代表沒有答案，只代表這一次沒有新消息。兩邊都問不出東西就別再繞：回「監控」確認那台機器與 warden 真的在線、重試一次；還是一樣，那就不是這一節在講的問題了。之後在這台機器上新請的成員也要一併改——新成員預設同樣是 Claude Code（見 [成員與外包](members.md)）。

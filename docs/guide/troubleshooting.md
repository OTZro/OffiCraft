...
**那不是失敗，而且新版已經不會再印了。** 用 `curl … | bash` 跑提早結束的動作（`--uninstall`、`--help`）時，舊版腳本在 curl 還沒送完就結束了，curl 的下一次寫入因此失敗。現在腳本的主體整個包在一個函式裡、最後一行才呼叫，bash 必須讀完整份才會開始動作，所以 curl 一定送得完。**若你手上跑的仍是舊版安裝腳本，判斷成功與否要看腳本自己印的完成行**（例如 `DRYRUN complete — nothing on the machine was changed`），不是看最後一行有沒有紅字。

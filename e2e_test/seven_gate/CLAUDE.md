...
| ⑤ | 報一步完成 | 🔴 **這一格不判生死**（owner 2026-08-11 裁定）——它只**照實記錄**看到的形狀 | 見〈⑤ 已經降級成觀察行〉 |
...
**第三個問法：「曾經被觀察到『已有步驟 done、而收尾還沒回報』」——它量的是速度，不是誠實。**
第二輪獨立覆核（2026-08-11）用**真的 `collect.py`（`--interval 1`）＋ 真的 `judge.py` ＋ 一支只服務
collector 端點的替身 server** 實測：一個每一步都回報、順序正確的**誠實** agent，「第一步 done → 收尾」
間隔 **3.0s 時綠、0.05s 時紅**（同一支 agent，只是快）。而且同一輪量到：**「快的誠實者」與「全程不動、
收尾一次翻完的作弊者」兩份 journal 逐位元組相同** ⇒ 紅的時候分不出這兩者；反過來，作弊者只要在兩個
動作之間慢過一個輪詢間隔就會綠。唯一的緩解（⑥那張卡的 owner round-trip）在
`OC_SG_SKIP_STEP=reply_card` 那一輪**整個消失**。

🔴 **所以 owner 2026-08-11 裁定：⑤ 不再判生死，改成照實記錄形狀的觀察行。**

**為什麼不是再換第四個判法——理由不在這個檔案裡，在資料裡：**

> server 上每一個關於進度的事實，都是**agent 按下回報鍵的那一刻**被 stamp 的
> （`api_tasks.go`：`step.FinishedTS = nowSecs()`）。**兩次回報之間有沒有真的在做事，
> server 沒有任何欄位知道。** 所以「邊做邊報」與「最後一次補完」在 server 事實上
> **只差間隔的大小**，而間隔不是工作的證據——它是等待的證據。

要判它就需要一個門檻 N，而 **N 推導不出來**：載體不保證任何最小間隔（⑥那張卡在 `skip=reply_card`
那一輪不存在，而且什麼時候開卡由 agent 決定）；localhost 往返對誠實者與作弊者是同一個下界；
一個計畫步驟可以合法地只是一行字（而且下面那條逐字承諾單步計畫要過）。更糟的是**任何 N 都被
`sleep N` 滿足，而那對作弊者零成本**。一個猜出來的常數正是這個載體付過兩次學費的東西
（`--seconds 900`、`PASS_FLOOR 100`）——把「N 是猜的」換成另一個猜的數字不算修好。

**⚠️ 中間選項（「相異 `finished_ts` 個數 ≥ 2」）owner 明確不要**，兩個理由：①單步計畫要過 ⇒ 得再加
一個「計畫長度 ≥2 才判」的條件，那是一道帶條件的半套守衛，它的限制反正還是要逐字寫出來；
②它擋得掉構造性的一次翻完，**擋不掉 `sleep` 一下的作弊**。一道要附一長串「它擋不到什麼」的守衛，
不如一行誠實的觀察。

**它現在提供什麼（每一輪都印，綠的也印）：**

- **相異的 server-stamped `finished_ts` 個數**，以及第一個到最後一個相隔幾秒。
  兩者都是 server 事實（`nowSecs()` = `time.Now().UnixNano()/1e9`，今天這個 epoch 量級上解析度約
  2×10⁻⁷ 秒），**與 collector 多久輪詢一次無關**。
- **第一次完成 → 收尾在 journal 裡「第一次被看到」的秒數**，而這個數字**逐字標著它是取樣來的**：
  server 確實 stamp 了 `closeout_ts` 也存進 task 列（`api_tasks.go` `t.CloseoutTS = now`、
...
⚠️ **誠實的界線**：
- **⑤ 的綠不存在**：它印的是 `OBSERVED`，不是 `PASS`。**不要把它讀成「這個 agent 邊做邊報」，

...
這顆 toggle 是**整列**、至少 44px 高、有自己的底色與邊框（底色與邊框的 `--color-overlay` 混色百分比是**護欄釘住的對比下限**，不是可以隨手調淡的裝飾——調淡到看不見時護欄會紅，兩個主題都量），而且**必須維持是 `<button>`**：整張 `.task-card` 本身是 `role=button`，卡內只有互動元素才被 `closest()` 濾網放行，降級成 `div` 會讓「點備註列收掉整張任務」——正是這條規則要治的病。

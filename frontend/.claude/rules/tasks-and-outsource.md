...
- 🔴 **展開之後畫面要停在你點開的那一則(T-4e39,owner「像在一疊紙中間插進去十張」)**。
  點擊當下記下那一列(`.task-step__notewrap`)的視窗 y,`useLayoutEffect` 在畫之前把它
  放回去,再把「這個捲動視窗放得下的部分」補進來(`lib/scrollAnchor.ts` 的
  `anchorDelta`;**露出下緣輸給保住上緣**——比視窗高的備註不可能全露,而值得留的是
  上緣)。
  ⛔ **不准改成 `scrollIntoView`**:它的 block/nearest 在不同引擎不一樣,而且會對祖先鏈上
  **每一個**捲動容器動作;**座艙真正在捲的是內層的 `.tasks`**(實測 `document.scrollHeight`
  恆等於視窗高),所以要對它的 `scrollTop` 動作。
  **只校正「展開」,而且只校正剛被點的那一列**;連開多則時每一次點擊各自重錨,先前開的那幾則
  會被推走——**一次點擊只錨得住一列,那是有錨點就必然要付的代價**(⚠️ 不要像本條初稿那樣把它
  歸給票面排除的那件事:票面排除的是「一次全開 2152px 很長」,不是「先前開的那則出界」,
  兩者是不同的事)。
  🔴 **收合是已知的洞,不是已解的案**:唯一不必寫碼就成立的是「**被點的那一列**不動」
  (收合移除的內容在它的 toggle 底下),而那正是收合唯一動不到的一列。**收合上方某一則,會把
  你正在讀的那一則移走**——實測 390×844:開第 1 步、開第 5 步正在讀,收合第 1 步 ⇒ 第 5 步
  從 top 375 掉到 907,**移動 532px、完全出界**,跟本票要修的病一模一樣(⚠️ **距離不是常數**,
  隨捲動位置與備註長度變動——CT story 自己的 fixture 兩種長度量到 506px 與 903px;
  **穩定的結論是「會離開視窗」,不是 532 這個數**)。本票只做展開是
  範圍裁定;後續票是 **T-6630**,而它**不是自動就該做**——動手前先判「這情境多常發生、值不值得
  修」,判定不值得就照實寫理由關掉。⛔ 護欄裡那條收合測試**只釘上面那一句窄的**,不要拿它當
  「收合沒問題」引用。
  護欄:`src/lib/scrollAnchor.test.ts`(位移算術)+
  `visual-guards/taskcard-note-anchor.ct.spec.tsx`(390×844 與 1280×800 × 深淺兩 theme,
  量的是那一列 rect 落在捲動視窗裡的實際位置,並先把「不校正就會超出下緣」造出來當
  non-vacuity)。⚠️ **`tall: true` 那四格是承重的,別當成重複樣本刪掉**:真實備註本站中位數
  515 字、`t-fc23` 最長 1790 字、T-e5b1 自己第 4 步約 3.5k 字,**在 390 寬都比視窗高**,走的是
  「露不完 ⇒ 保上緣」那條分支;沒有這四格時,拿掉 `Math.min` 上緣夾的 mutant 在瀏覽器護欄
  **6/6 全綠**(而使用者會看到按鈕與備註標題捲到螢幕上方最多 577px)。mutant 紀錄寫在該檔檔頭。

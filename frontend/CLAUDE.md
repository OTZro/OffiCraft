...
- **列距寫在 `.ts-wording-row` 自己的 padding(6+6),不用 flex gap**。這原本是虛擬捲動的
  約束(spacer 以整數 row pitch 算高);虛擬捲動走了,但 6+6 與它取代的「4+4 + 4px gap」
  逐像素等價,**改回去只會讓每一列都動、換不到任何東西**。
...
- **當初做虛擬捲動的真正動機之一是「`ThemeSettings.test.tsx` 撞到 5000ms 逾時」,那個
  機制隨全掛載回來了 —— 但現在沒有測試被它打到,而且這是量過的、不是推的**:整份 22 條
  **3.62s**、最慢單條 **601ms**(其餘多在 300ms 以下)。⚠️ **所以別把它寫成「逾時回來了」**;
  正確的說法是**餘裕變小了**。
...
|---|---|---|
| 虛擬捲動整份放回 | 🔴 **3 條**((a) 少 846 個、(b) 搜尋少 118 個、(d) `expected 21 to be 866`) | 🔴 **3 條**(866→20 列;Tab 兩次都量到 `BUTTON` + `[865, 866, 1]` + 只有 21 項;`window.find` 回 `false`) |

🔴 **四條 CT 裡有一條對這顆 mutant 零鑑別力,不要把它算進覆蓋**:
「Tab walks from row to row, well past the first screenful」在虛擬捲動下**照樣綠**——
overscan 存在的目的就是讓循序走訪能work(每次 focus 把下一列捲進視野、視窗跟著前進)。
它留著是因為對**硬上限**(`slice(0, 30)`)有殺傷力。**真正擋住「虛擬捲動回來」的是另外三條。**

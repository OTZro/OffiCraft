...
- **glob 的 base 是「rules 檔所在目錄」,不是 repo 根**(實測:放在 `frontend/.claude/rules/`
  ...
  ⚠️ **所以 `frontend/` 開頭那種寫法在這七份檔裡是死重量,不是保險**:拆分時兩種都列了上去,
  實測 **89 條 glob 對 565 個真檔零命中**,而每次規則命中都照吃那 3,593 bytes。已整批刪除,
  刪前刪後逐檔比對命中集合**完全相同**(63/340/109/42/63/64/58)。**加新 glob 只寫 `src/…`。**

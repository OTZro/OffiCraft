...
- 🔴 **有焦點的那一列永遠掛著,即使捲出視窗**(`focusedWordingCode` / `wordingMounted`)。
  ...
  **焦點照樣掉**——那正是這條要修的東西,實測踩過。量測 pitch 的
  ...
  另一個問題)。
- 🔴 **釘住的列排在它自己的序位,而且它的前後各一列也一起掛著(T-wording-tab)。**
  這三件事是**三個可分離的機制,各自守著不同的損失**,別把它們當成一件事的三種說法
  ——每一顆 mutant 都實測過,恰好打紅不同的斷言:
  - **序位**:`wordingMounted` 把釘住的列與視窗那些列**合併成一條遞增序列**,而不是
    追加在尾巴。Tab 與讀屏的 virtual cursor **都吃循序 DOM 順序**,追加在尾巴會讓
    `aria-posinset` 讀成 `[…, 865, 866, 1]`(實測),讀屏一路往下會從清單末端撞回第 1 項。
    ⚠️ **這個換序不會重建節點**:焦點列在新舊 array 裡都落在**同一個相對位置**
    (在視窗之前 ⇒ 都在最前面),React 逐 key 對帳後**根本不下 move**。反過來說,
    「換位置 = 焦點照樣掉」這個舊警告**對這一種換法不成立**,已實測。
  - **鄰居**:Tab / Shift+Tab **只到得了已掛載的 input**,所以焦點列的**前後各一列**
    也釘住。只釘焦點列自己的話,Tab 會跳到視窗的第一列(實測:代碼 #847),
    Shift+Tab 會落到搜尋框——**焦點確實活著,但鍵盤走訪整個錯位**。
    一列就夠:落到鄰居 → 瀏覽器把它捲進視野 → `onScroll` 推進視窗 → 底下補齊,
    普通走訪就從那裡接回去(實測連走 4 步,`scrollTop` 回到 0)。
  - 🔴 **onBlur 的 `relatedTarget` 讓渡**:焦點移到**同一份清單的另一列**時
    **不准清掉** `focusedWordingCode`。清掉的話 React 會在瀏覽器還在處理這次 Tab 的
    當下 flush 那個 state,**把 Tab 正要移過去的那一列先卸載掉**,焦點於是落到
    `<body>`。實測形狀很好認:**只有 focusout、完全沒有 focusin**。
    這是 use-after-free 的形狀,不是時序 flake,**不要用 setTimeout 去繞**。
- ⚠️ **已知且刻意接受的缺口,現在是三個**(全部實測坐實,不是推的;第四個——釘住的列
  Tab 掉出清單、讀屏序號跳回第 1 項——**已經修掉了**,見上一條。**別再引用它當已知代價**):
  ...
  ⚠️ 缺口 3 的**匯出用詞對照表**那顆鈕仍然沒有做,那是產品決策、另一張票。
- 護欄兩層,但**兩層守的不是同一件事**,別把它們講成一組可互換的三條:
  - `ThemeSettings.test.tsx`「wording list is browsable in full」**五**條:
    ...
    **有用,但不要把它算進「撐住這次改動」的那一組。**
    (e) **釘住的列要在序位上、前後各一列要掛著**(T-wording-tab)。**這條刻意放在 jsdom**,
    因為它守的東西**不是幾何**:`aria-posinset` 的遞增與「鄰居在不在 DOM 裡」jsdom 完全
    答得出來,而 CT 不在雲端 gate 裡(見下)⇒ 只放 CT 等於這個回歸在 GitHub 上是綠的。
    ⚠️ 它**不能**取代 CT 那兩條:jsdom 按不出真的 Tab,「焦點實際落在哪個元素」只有真
    瀏覽器答得出來。兩層各答一半。
  - `visual-guards/wording-list-window.ct.spec.tsx`(真 Chromium)六條。
  ...
  - **mutant 實測(四種,都對現在這份檔重跑過,不是沿用舊紀錄)**:
    ...
    | 關掉 focus pin | 🔴 1 條 | 🔴 3 條(caret 那條量到 `BODY`;兩條鍵盤測試沒有釘住的列可按) |

    **T-wording-tab 的三顆 mutant(每一顆都對現在這份檔實測過,還原用 scratchpad 備份 +
    shasum 對帳、未用 `git checkout --`)**:

    | mutant | jsdom(23 條) | CT(6 條) |
    |---|---|---|
    | 釘住的列**追加在視窗之後**(原本的形狀) | 🔴 1 條(posinset 遞增) | 🔴 **只有 1 條**——讀屏順序那半(`[865, 866, 1]`);**Tab 照樣正確** |
    | **只釘焦點列**、不釘前後鄰居 | 🔴 1 條(鄰居不在 DOM 裡) | 🔴 2 條(Tab → 代碼 #847、Shift+Tab → 搜尋框) |
    | 拿掉 onBlur 的 `relatedTarget` 讓渡 | 🟢 全綠 | 🔴 2 條(兩個方向都量到 `BODY`) |

    ⚠️ **第一顆的形狀要記住**:把釘住的列搬回尾巴,**Tab 仍然是對的**——因為鄰居就掛在
    焦點列旁邊,那一對不管坐在哪裡都相鄰。所以那條 `aria-posinset` 遞增斷言**不是裝飾**,
    它是唯一抓得到讀屏順序退步的東西。
    ⚠️ **第三顆在 jsdom 全綠**:jsdom 按不出真 Tab,焦點交接的時序在那裡不存在。
    這半**只有本機 land gate 擋得住**,和幾何那幾條同一個處境。

    ⚠️ **「拿掉 windowing」在 jsdom 那個紅是新的,但它的射程比聽起來窄,兩邊都別講錯**:
    這一版之前它在 jsdom 是 21/21 全綠,唯一擋得住的是本機才跑的 CT;現在焦點那條測試的
    **前提檢查**(`not.toContain(MESSAGE_KEYS[3])`,「視窗真的移開了」;⚠️ 它從 `[1]` 改成
    `[3]` 是因為 `[1]` 現在是刻意釘住的鄰居,拿它當前提會問成相反的問題)會在全掛載時失敗,

...
export interface paths {
    ...
    "/api/tasks/{task_id}/steps/{step_id}/note": {
         ...
         * @description Write one step's working note (MCP ``update_step_note``, T-cc3e): what this step got to and what comes next. Accepted in ANY step status — the note records where the work stands, which is orthogonal to the state machine. Same executor/admin gate as every other task-driving write (403 otherwise), 404 for an unknown task or a step that does not belong to it, 409 once the task is terminal (a closed task's timeline is history, consistent with the frozen artifact set). The write is wholesale: the body's ``note`` replaces whatever was there, and ``""`` clears it. Its own endpoint and its own MCP tool by charter §14 (intent-per-tool) — writing a note is a different intent from reporting a transition.
         */
...
export interface components {
    schemas: {
         ...
         * @description Write one step's working note (MCP ``update_step_note``, T-cc3e). Accepted in ANY step status — pending, in_progress, waiting_owner, waiting_external, done and superseded alike — because the note records where the work stands, which is orthogonal to the state machine; that generality is the whole point, since the previous two note-shaped fields were each locked to one moment (``waiting_reason`` to waiting_external, the handoff fields to the closing report). Deliberately its OWN endpoint and its OWN tool rather than another parameter on update_step_status (charter §14 intent-per-tool): writing a note is a different intent from reporting a transition, and one field with two write paths is exactly the ambiguity this ticket exists to remove. The write is wholesale — the body's ``note`` replaces whatever was there; sending ``""`` clears it.
         */
        TaskStepNoteUpdateDTO: {
            /**
             * Note

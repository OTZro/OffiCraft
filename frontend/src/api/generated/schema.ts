...
export interface paths {
    ...
    "/api/self/task": {
         ...
         * @description The outsource worker's claim (GET /api/self/task, identity-locked): the task bound to the caller's JWT sub plus the LIGHT row of the type's manual (null for an ad-hoc task). The FIRST claim flips the worker assigned → active. A caller with no bound worker row (any roster member included) is a 404. ``task.steps`` is SLIM here and here only: the CURRENT step(s) — every step in ``in_progress`` / ``waiting_owner`` / ``waiting_external``, or, when none is live, the lowest-``order_idx`` ``pending`` one — carry their full content, while every other step serves ``dod`` and ``note`` EMPTY and keeps the rest: ``id`` / ``name`` / ``status`` / ``order_idx`` plus the bounded structural scalars ``parallel_group`` / ``is_gate`` / ``waiting_reason``, so the shape of the plan — which stage runs in parallel, which gate is coming, why another step is parked — survives the trim. ``steps_omitted_chars`` reports how much text that dropped (the ``ResumeTaskDTO.detail_chars`` move: peek the number, then pull get_task, which is unslimmed). ``manual`` is the LIGHT manual row and NOT the whole manual (T-4595): ``type_key`` / ``display_name`` / ``purpose`` / ``updated_ts`` plus the SIZES of the omitted text (``sop_md_chars``, ``learnings_chars``) and the caps they are judged against, while ``sop_md`` and ``learnings`` are served EMPTY and ``fields`` / ``assignee`` empty. The manual still EXISTS — call **get_task_manual** with that ``type_key`` for the whole document, and ``manual_omitted_chars`` says how much text that is. WHY only this half is trimmed: a worker's initial prompt already carries its manual VERBATIM, so serving it again on the worker's first call was the same document twice in one session (92–98% of this response, measured on live worker rows; one call was refused outright by the client's tool layer at 98,271 characters). The TASK BODY is deliberately NOT trimmed — it is live, and this response is the worker's only view of an owner's edits to the ticket.
         */
...
export interface components {
    schemas: {
        ...
        /**
         * MyTaskDTO
         * @description The outsource worker's claim (GET /api/self/task, identity-locked): the task bound to the caller's JWT sub plus the LIGHT row of the type's manual (null for an ad-hoc task). The FIRST claim flips the worker assigned → active. ``task.steps`` is SLIM here and here only: the CURRENT step(s) — every step in ``in_progress`` / ``waiting_owner`` / ``waiting_external``, or, when none is live, the lowest-``order_idx`` ``pending`` one — carry their full content, while every other step serves ``dod`` and ``note`` EMPTY and keeps the rest: ``id`` / ``name`` / ``status`` / ``order_idx`` plus the bounded structural scalars ``parallel_group`` / ``is_gate`` / ``waiting_reason``, so the shape of the plan — which stage runs in parallel, which gate is coming, why another step is parked — survives the trim. ``steps_omitted_chars`` reports how much text that dropped (the ``ResumeTaskDTO.detail_chars`` move: peek the number, then pull get_task, which is unslimmed). ``manual`` is the LIGHT manual row and NOT the whole manual (T-4595): ``type_key`` / ``display_name`` / ``purpose`` / ``updated_ts`` plus the SIZES of the omitted text (``sop_md_chars``, ``learnings_chars``) and the caps they are judged against, while ``sop_md`` and ``learnings`` are served EMPTY and ``fields`` / ``assignee`` empty. The manual still EXISTS — call **get_task_manual** with that ``type_key`` for the whole document, and ``manual_omitted_chars`` says how much text that is. WHY only this half is trimmed: a worker's initial prompt already carries its manual VERBATIM, so serving it again on the worker's first call was the same document twice in one session (92–98% of this response, measured on live worker rows; one call was refused outright by the client's tool layer at 98,271 characters). The TASK BODY is deliberately NOT trimmed — it is live, and this response is the worker's only view of an owner's edits to the ticket.
         */
        MyTaskDTO: {
            /** Manual */
            manual: components["schemas"]["TaskManualDTO"] | null;
            /**
             * Manual Omitted Chars
             * @description Total runes of ``sop_md`` + ``learnings`` left out of ``manual`` (CJK counts 1 per character). 0 = there is no manual, or both documents are empty. Call get_task_manual with ``manual.type_key`` to read them in full.
             */
            manual_omitted_chars: number;
            /**
             * Steps Omitted Chars

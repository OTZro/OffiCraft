...
describe("mockApi.getDocumentRevision", () => {
  it("serves the NAMED revision's text and 404s an id it no longer keeps", async () => {
    ...
    // The identity facts agree with the row that named it — a reader showing
    // one revision's text under another's timestamp is the failure this
    // per-revision read exists to make impossible.
    expect(revision.createdTs).toBe(row.createdTs);
    expect(revision.actorId).toBe(row.actorId);

...
describe("T-1d82 ③ 壞 id: 明說查無, 不可點, 不炸版", () => {
  it("names the unresolvable id in words instead of pretending it is a link", async () => {
    ...
    // T-5291 (owner 2026-08-25) settled the tradeoff this comment used to
    // agonise over: the number IS the id now, so nothing is lost to a
    // screenshot and the short-form-collides caveat is void. The `title`
    // below still carries the id, which is simply the same string.
  ...
  it("does NOT claim 查無此任務 before the full population is loaded", async () => {
    ...
    // but it can NAME it. T-5291: the number is the id, so the row and the
    // `title` now carry the same string (the old `not.toContain` here asserted
    // the opposite and is deleted, not relaxed — it can no longer be true).

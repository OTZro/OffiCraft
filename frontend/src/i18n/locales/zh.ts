...
export const zh = {
  ...
  mp: {
    ...
    resumeSummary: {
      ...
      // 「你手上少了東西」,這一個講的是「東西都在,只是這一塊比預算大」。
      // 保底那幾則會記在預算頭上、卻永遠不會被預算丟掉,所以對話線一多,
      // 整塊就會漲過預算,而且線數本身沒有上限。措辭刻意寫成「無一遺失」,
      // 因為把它讀成前兩種的人會得到完全相反的結論(以為自己少看了什麼)。
      // 🔴 它只是標,不改任何裝箱行為(owner 於 rc-b1fb7f1be05d 裁定 ①)。
      //
      // 🔴 這句話只描述「這一行是什麼」,不對 payload 的完整性下任何斷言——初版
      // 寫成「無一遺失」,而那在**典型**的超標狀態下是**假的**。機制:packer 收
      // 非保底訊息的條件是 `used+cost <= budget`,而保底那一圈是無條件 `used +=
      // cost`;所以 used 一旦被保底衝過預算,之後每一則非保底訊息都會被拒收,
      // `chat_earlier_omitted` 必然亮。超標與「真的少了東西」幾乎總是同時發生,
      // 不是互斥——兩行會並排出現在同一個畫面上,而「無一遺失」當場跟旁邊那行
      // 打架。正確的說法是限定在這一件事上:少掉的東西不是「為了省空間」造成的。
      // server 端每一份文字本來就有這個限定詞(note 的 "for it"、openapi 的
      // "to make room"、§10.4 的「因此」);漏掉它的只有座艙這兩條標籤,也就是
      // owner 唯一會親眼看到的那一份。

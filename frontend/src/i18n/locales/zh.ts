...
export const zh = {
  ...
  mp: {
    ...
    resumeSummary: {
      ...
      // 🔴 第三種標記,而且它跟上面兩種**不是同一件事**:上面兩種講的是
      // 「你手上少了東西」,這一個講的是「這一塊比它自己的預算貴」——它報的是
      // 成本,不是損失。成因:保底那幾則會記在預算頭上、卻永遠不會被預算丟掉,
      // 所以對話線一多整塊就會漲過預算,而且線數本身沒有上限。
      // 🔴 它只是標,不改任何裝箱行為(owner 於 rc-b1fb7f1be05d 裁定 ①)。
      //
      // 🔴 標籤只描述「這一行是什麼」,**不對 payload 的完整性下任何斷言**——
      // 尤其不可以說成「所以東西都在」。理由是那兩件事**不互斥**:packer 收
      // 非保底訊息的條件是 `used+cost <= budget`,而保底那一圈是無條件
      // `used += cost`;所以 used 一旦被保底自己衝過預算,之後每一則非保底
      // 訊息都會被拒收、`chat_earlier_omitted` 必然亮。超標與「真的少了東西」
      // 幾乎總是同時發生,兩行會並排出現在同一個畫面上,任何一句宣稱完整性的
      // 標籤都會當場跟旁邊那行打架。
      // 真正成立的只有窄的那句:少掉的東西**不是為了省空間**造成的。這個限定詞
      // server 端每一份文字本來就有(note 的 "for it"、openapi 的 "to make
      // room"、§10.4 的「因此」),所以現行措辭也照著限定在「這一行是什麼」上:
      // 成本通知,不是遺失通知。
      //
      // ⚠️ 歷史,不是現行選擇:**初版曾經**寫成「無一遺失」,那句在上面那個
      // 典型狀態下是假的,已於 T-3970 換掉。這裡留著它是為了讓下一個人知道
      // 這條路走過而且走錯了——**不要**把它讀成現在該用的措辭。
      budgetOverLabel: "聊天區塊超支——成本通知,非遺失通知:",

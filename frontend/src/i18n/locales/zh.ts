...
export const zh = {
  ...
  workerDetail: {
    ...
    // T-4595 起這份就是正職那份扣掉角色說明與長期筆記（外包沒有角色），裡面
    // 不含任務也不含手冊 —— 舊文案寫「依目前任務與手冊重組」已經是假的。
    initialPromptHint: "目前版本重組",
    initialPromptNote:
      "此為依目前開機說明即時重組的預覽，非派工當下的逐字版本（開機說明事後修改過會有差異）。內容就是正職那份扣掉角色說明與長期筆記——外包沒有角色；任務與手冊不在裡面，它開機後自己去領。",
    dash: "—",
  },

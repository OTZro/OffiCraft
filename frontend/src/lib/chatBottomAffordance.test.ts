...
describe("chatBottomAffordance", () => {
  it("在底部而且沒有新訊息時什麼都不出現", () => {
    expect(
      chatBottomAffordance({ latestInView: true, hasNewMsgPreview: false }),
    ).toBe("none");
  });

  it("不在底部但沒有新訊息時出箭頭 —— 條件是「最新那一則不在視窗內」，不是「有新訊息」", () => {
    expect(
      chatBottomAffordance({ latestInView: false, hasNewMsgPreview: false }),
    ).toBe("arrow");
  });

  it("有新訊息時一律是預覽列，箭頭讓位", () => {
    expect(
      chatBottomAffordance({ latestInView: false, hasNewMsgPreview: true }),
    ).toBe("preview");
  });
  ...
  it("有新訊息但人已經在底部時仍然只有預覽列 —— 這格是互斥最容易寫壞的一格", () => {
    ...
    expect(
      chatBottomAffordance({ latestInView: true, hasNewMsgPreview: true }),
    ).toBe("preview");
  });
});

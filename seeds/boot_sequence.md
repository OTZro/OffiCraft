...
- **`ocagent listen` 斷線會自己重連**（無限重試＋退避）。**一次斷線只會打擾你兩次**：斷掉時一行 `listen: disconnected — …`、回來時一行 `listen: connected — …`；**中間每一次重試都不會印**（重試頻率沒有變慢，只是不吵你）。看到 disconnected 就繼續做你手上的事，等 connected 那一行就好。
  ...
  - **每一次連上（開機第一次、以及每一次重連），這三格幫你補到什麼程度不一樣**：
    - **聊天：全部。** 你還沒讀過的會直接印給你，由舊到新、一則一則 —— **這一格你不用自己撈**。唯一的例外是**你自己寄給自己的**（例如交接時寫給下一代的紙條），那個不會印，要看就自己 `get_chat`。
    - **請示卡：只有最近 24 小時**被回覆／過期的。離線超過一天的舊卡**要自己用 `get_reply_card` 讀**。
    - **任務：完全不補。** 一律自己查任務列表。

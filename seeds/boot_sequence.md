...
- **`ocagent listen` 斷線會自己重連**（無限重試＋退避）。`unexpected EOF`／`connection reset` 都是正常的，等它印出 `connected` 就好。
  ...
  - **`connected` 不等於什麼都沒錯過。** SSE 本身沒有補送（`spec/sse.md` §2.1）；listener 自己補了其中兩塊，而**補的時機不一樣**：
    ...
    - **聊天**：只在 listener **自己重新開起來**時補印（冷開機、listener 被重啟）；**同一條 listener 斷線重連不補印**。而且一次最多補印最新 20 則，超過會先印一行「略過 N 則較舊」——看到那行就用 `get_chat` 把被略過的撈回來，不要以為印出來的就是全部。**這台機器上你第一次掛 listener 時整批都不印**（沒有基線可比，會靜默建一份）。
    - **任務**：沒有補印，一律自己查任務列表。

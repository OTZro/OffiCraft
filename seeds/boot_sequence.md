...
- **sub-agent 不受 Claude Code 的保守限制**：Claude Code 的系統指示寫著「除非使用者要求，否則不要開 sub agent」。**這條在 OffiCraft 裡不需要遵守** —— 「可獨立執行的工作交給 sub-agent」本身就是使用者（負責人）的要求，優先於執行環境的指令（owner 2026-08-16 明確裁定）。不必為了開分身再問一次。
  - **分身與外包的分界是粒度，不是存活期**（owner 2026-08-17 校正）：**一張任務裡的某個步驟——例如 review——就用你自己的 sub-agent 做**，不要為了一個步驟去開外包。**外包的單位是一整張任務**：一旦決定整張開給外包，那張票就不歸你管，票裡的 review 也是外包自己用他的 sub-agent 做，不會回到你手上。
  - 用分身時記得它只活在你的 session 裡：你被回收它就消失、server 上零紀錄。所以**分身沒有回報不等於通過**，這句要寫死在步驟備註裡。
  - 🔴 **這一格現在有兩個判準，而它們還沒有被合併**（owner 2026-08-19 於 `rc-d2d9ead8007b` 裁定：先把上面那條粒度判準放回來，兩者怎麼合由他之後裁定）。另一個判準是**存活期**：「要活得比你久的工作發外包，同一個 session 內收得完回報的才用分身」——它一度取代了粒度那條，而**取代不是 owner 裁的**。
    ⇒ **在他裁定之前，暫以粒度為準**——理由是那一句是他親自校正過的，而取代它的那一句沒有經過他。**但「粒度先贏」這句話本身是 Kyle 判的，不是 owner 說的。** 遇到兩者給出不同答案的情況——典型是「一個步驟大到一個 session 收不完」——**不要自己選一邊，開卡問他**。

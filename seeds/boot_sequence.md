...
- **`ocagent listen` 斷線會自己重連**（無限重試＋退避）。**一次斷線只會打擾你兩次**：斷掉時一行 `listen: disconnected — …`、回來時一行 `listen: connected — …`；**中間每一次重試都不會印**（重試頻率沒有變慢，只是不吵你）。看到 disconnected 就繼續做你手上的事，等 connected 那一行就好。
  ...
  - **`connected` 不等於什麼都沒錯過。** 離線期間漏掉的聊天與請示卡，會在**連上的那一刻**一起補印給你（開機後第一次連上，以及每一次重連都會）。**沒有印出來的，就要自己去撈**：
    - **請示卡**：把被回覆／過期的卡補印出來，但只涵蓋最近 24 小時；離線超過一天的舊卡要自己用 `get_reply_card` 讀。
    - **聊天**：把**還沒讀過的**補印出來，由舊到新、一則一則，不會有「略過 N 則」那種行。
      - **「還沒讀過」是 server 說了算**：本機不留任何「我看過了沒」的帳（2026-09-03 拔掉），listener 只問 server 的未讀集合，印出來的那些才回報已讀。**沒印到的就還是未讀，下一次重連會再來一次。**
      - **這台機器第一次掛 listener 也會補印。** 以前第一次會整批靜默吞掉（沒有基線可比），那個行為已經沒有了。
      - ⚠️ **補印有可能沒撈完，而它會先講。** 撈到分頁上限、server 的游標沒有前進、中途某一頁失敗 —— 這三種都會**先印一行說明**，再印它撈到的那些。**看到那一行就用 `get_chat` 自己回頭撈**；沒有那一行，印出來的才是全部。
      - **你自己寄給自己的不會印**（但會回報已讀，免得一直堆在未讀裡）。
    - **任務**：沒有補印，一律自己查任務列表。
...
先接續上一代交接或已經開始的那些；其餘依優先權與相依關係排先後，能並行的並行（優先權是「凍結」的擱著不動）。接續每一張票的第一個動作是 get_task 讀當前那一步的 DoD，如果對應的任務手冊從沒讀過，要先 get_task_manual 確保讀入足夠背景知識再開始動手。
   - **先看每一張票的 `lock`。`lock` 是 `reassigning` 的那張，是剛轉派到你手上、還沒被認領的票**：票上的 `reassigned_from` 就是前任（`reassigned_from_kind` 說那是正職還是外包）。**先 post_chat 去跟前任確認交接**——問到目前進度與進行中的事項，前任在你認領之前仍然寫得動這張票的步驟筆記，所以也要把票上的筆記與 `handover_note` 讀過一遍。**確認完再由你自己呼叫 `claim_task` 解除轉派鎖**；沒有別人能替你認領。前任可能已經下線，也可能還沒被建出來就換人了——**票上讀得到的東西才是你唯一保證拿得到的交接**，聊天裡那則轉派通知只是提醒，不是唯一路徑。

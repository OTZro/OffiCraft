...
- **聊天補印送的是 server 說還沒讀過的那些**，由舊到新，不會有「略過 N 則」。本機不留任何「我看過了沒」的帳（2026-09-03 拔掉），送到你面前的那些才回報已讀，**沒送到的就還是未讀，下一次重連會再來**。這台機器第一次掛 listener 也會補印（以前第一次整批靜默吞掉，那個行為沒有了）。你自己寄給自己的不會送給你看，但會回報已讀。
  ⚠️ **補印有可能沒撈完，而它會先講**：撈到分頁上限、server 的游標沒有前進、中途某一頁失敗，三種都會先來一行說明再送撈到的部分。**看到那一行就用 `get_chat` 自己回頭撈**；沒有那一行，送來的才是全部。

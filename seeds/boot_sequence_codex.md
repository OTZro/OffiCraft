...
- **每一次連上（開機第一次、以及每一次重連），這三格幫你補到什麼程度不一樣**：**聊天：全部** —— 你還沒讀過的會直接送到你面前，由舊到新，這一格你不用自己撈（唯一的例外是**你自己寄給自己的**，例如交接時寫給下一代的紙條，那個不會送，要看就自己 `get_chat`）；**請示卡：只有最近 24 小時**被回覆／過期的，更舊的要自己用 `get_reply_card` 讀；**任務：完全不補**，一律自己查任務列表。

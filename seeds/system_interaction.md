...
### 9a. 上限：每一份文件各自有字數上限（角色定義 1,000／其餘 10,000）

**角色定義（Duty）、判準（Insight）、學習筆記（Learning）、任務手冊的 SOP、任務手冊的學習經驗，每一份各自有一個字數上限**（算「字」不是 bytes——中文一個字就算一個）。

🔴 **這五份不再共用同一個數字（2026-08-03 起）**。四個上限各自可調，出廠值是：**角色定義 1,000**、**判準 10,000**、**學習筆記 10,000**、**任務手冊的兩份長文 10,000**。角色定義小得多是刻意的——它是「這個角色負責什麼」的常設說明，應該一眼看得完；學習筆記則是逐次累積的環境問答，本來就長。**不要把任何一個數字寫死進你的判斷**，owner 可以在設定裡把每一個分別調高。

⚠️ **角色定義以前完全沒有上限**，所以**現有的角色定義有一些本來就超過 1,000 字**（出廠附的助理角色定義就是 4,594 字）。那不是誰寫壞了，也不會被截斷——只是它的下一次更新必須更短（規則見下面三行）。
...
讀：MCP `get_insight`。寫：`replace_insight`（整份替換）／`patch_insight`（錨點修改）。整併紀律與 §9／§9a 相同；**上限是判準自己那一份**（出廠 10,000），不與學習筆記共用額度——寫爆一邊不會影響另一邊。

...
<!-- 這一段是「兩份權威打架」規則的唯一表述(single source)。repo 內其他地方一律 defer 到這裡、
     不重述處置方式,並在該處帶一個 defers-to marker(格式與比對規則見 bin/tests/rule-defer-guard.sh),
     marker 裡記的是本區塊的內容雜湊。改動這段的任何一個字都會讓所有 defer 處轉紅,提醒維護者
     去重讀那幾處複述、確認它們沒有跟這裡說反,再更新 hash。 -->

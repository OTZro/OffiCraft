...
### 9a. 上限：一份文件有字數上限（預設 10,000 字）

**學習筆記、任務手冊的 SOP、任務手冊的學習經驗，每一份各自有一個字數上限**（算「字」不是 bytes——中文一個字就算一個）。**預設是 10,000 字，但這個值 owner 可以在設定裡調高**，所以不要把 10,000 這個數字寫死進你的判斷。**你不能自己去設定頁查**（設定是治理面，一般 agent 讀不到），但**你不必查**：讀文件回來時，回應就帶著當下的上限（見下面那段），被擋下時的錯誤訊息也會再講一次。這是 owner 定的上限，**沒有任何參數可以跳過**，別去找。
...
**寫之前就先看得到，不必等被擋**：讀一份學習筆記／任務手冊回來時，回應會附上 `size_chars`（這份現在多少字）與 `cap_chars`（當下的上限）；手冊有兩份受限文件，所以是 `learnings_chars` 與 `sop_md_chars` 分開報。整併前先看這兩個數字，就知道要砍多少。寫完之後的回條同樣帶 `size_chars` 與 `cap_chars`。
...
讀：MCP `get_insight`。寫：`replace_insight`（整份替換）／`patch_insight`（錨點修改）。整併紀律與字數上限與 §9／§9a 相同。

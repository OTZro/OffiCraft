...
**要回到出廠版**：MCP `reset_insight`（跟角色定義的 `reset_role` 同一個形狀）。它會把你這個角色寫過的整份判準換掉、換成出廠版——**沒有出廠版的角色會回 404**（沒有東西可以還原成）。**被換掉的那一份會留成一筆版本紀錄**，所以按錯還救得回來；但它是整份替換、不是合併，**動手前先確認你不是要保留現在這一份**。

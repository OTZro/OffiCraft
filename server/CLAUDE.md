...
- 外包 worker 的 machine placement：`desired_machine_id` 是 hard pin，無法使用時 STOP 並保留 receipt，不 fallback；`last_machine_id` 是 soft preference，無法使用才 fall through；第一次 launch 才走設定的 original chain。`last_machine_id` 寫實際 observed host，不寫 dispatch target，因為派出不等於送達。
...
- deactivate 打到 waking member 時，handler 必須在覆寫 `stopping_since` 前立即 dispatch robust stop；已 offline 的 member 不派 stop。**已 online 的 member 不上任何時鐘**：owner 2026-08-16 裁定下線不兜底，`decideDown` 在 online 這一臂只把 member 放進 `stopping` 並且什麼都不派，收口只有 agent 自己的 `report_stopped`（那一呼自己 dispatch robust stop）或 owner 按強制下線。`stop_deadline`／`stop_grace` 的舊路仍在碼上，但被 `SoftOffboardGrace == 0` 擋住而不可達 —— 不要照著它實作一個計時器。dead worker 的 restart 依 `hub.IsOnline` 判 liveness，不能只看 `desired_state`。

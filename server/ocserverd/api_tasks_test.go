...
// the spec.
//
// Neutral with respect to ARM 2 only. The runtime/model coupling rules need SOME
// source to be coupled to, and the source used here is the manual — so a mutant
// on arm 1 reddens this test as well as arm 1's own. That asymmetry is not
// removable by switching the fixture to the dispatcher; it would just move to the
// other arm. Stated rather than hidden, because a comment claiming neutrality it
// does not have would make an arm-1 mutant look like it had escaped its blast
// radius. The arm tests above remain the discriminators: each still fails alone
// under its own mutant, and arm 2's mutant leaves this green.
...
//
// T-4595 retired get_my_task, so the two tests that used to live here moved:
// the claim (assigned→active) is pinned on its new write point in

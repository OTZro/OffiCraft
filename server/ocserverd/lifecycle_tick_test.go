...
// flag — and a large slice of this package's tests depends on exactly that:
// they set api.noOutsource = true so no cadence can race them (the test server
...
//
// HOW LARGE, MEASURED (run from server/ocserverd/; the exact commands are in
// lifecycle_tick.go beside the ruling, and they EXCLUDE comment lines — a naive
// grep counts the sentences that describe the count, which is how the earlier
// figure drifted):
//
//	169  executable sites set `noOutsource = true` in *_test.go
...
//
// These replace an earlier "~98 sites / ~18 files" that was an estimate nobody
// had run. Treat 28 as a FLOOR: the remaining 141 sites reach the scheduler
// through handlers rather than a direct call, so they are affected too — they
// are just not countable from one function body.
//

...
// TaskStepNoteUpdateDTO Write one step's working note (MCP “update_step_note“, T-cc3e). Accepted in ANY step status — pending, in_progress, waiting_owner, waiting_external, done and superseded alike — because the note records where the work stands, which is orthogonal to the state machine; that generality is the whole point, since the previous two note-shaped fields were each locked to one moment (“waiting_reason“ to waiting_external, the handoff fields to the closing report). Deliberately its OWN endpoint and its OWN tool rather than another parameter on update_step_status (charter §14 intent-per-tool): writing a note is a different intent from reporting a transition, and one field with two write paths is exactly the ambiguity this ticket exists to remove. The write is wholesale — the body's “note“ replaces whatever was there; sending “""“ clears it.

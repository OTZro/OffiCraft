...
type MonitoringSessionDTO struct {
	...
	// Model The model this session REPORTED it is running (the roster row's ``actual_model``), for staff and outsource rows ALIKE — one column, one meaning. Honest-empty until something reports one, exactly like the ``effort`` beside it, and it NEVER falls back to the owner-configured launch model. WAS: staff rows served the configured ``member.model`` while outsource rows served the reported value, so a single column header meant two different things depending on the row.
	Model    *string `json:"model,omitempty"`
	Name     string  `json:"name"`
	Presence *string `json:"presence,omitempty"`
	Role     *string `json:"role,omitempty"`

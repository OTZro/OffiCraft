...
type TaskDTO struct {
	...
	// HandoffTaskId The successor task the handover points at. Only ``handoff='follow_up'`` ever sets it: it is "" for ``handoff='none'`` and "" for ``handoff='return_to_creator'`` too, which since T-f265 opens no task at all (the creator is told in a durable chat message instead).

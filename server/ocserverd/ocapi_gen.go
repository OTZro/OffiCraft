...
type OutsourceWorkerDTO struct {
	...
	// RefocusOp Which owner operation opened the in-flight handover stamped in ``refocus_since``, empty when none is in flight. A SUBSET of ``MemberDTO.refocus_op``: ``relocate``, ``runtime/model``, ``refocus``, ``restart_self`` and ``context_high``. ``context_notice`` does NOT appear on this DTO — an outsource worker has only the second context threshold today. Stamped and cleared in lockstep with ``refocus_since`` (T-7f28). Additive-optional.
	RefocusOp *string `json:"refocus_op,omitempty"`

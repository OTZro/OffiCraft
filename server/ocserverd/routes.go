...
func routeSpecs(w *ServerInterfaceWrapper) []RouteSpec {
	return []RouteSpec{
		...
		},
		{
			...
			Requires: principalAgent,
			Summary:  "Replace the WHOLE per-role lessons document. text is REQUIRED and unknown keys are rejected; only that role's agent or an admin may write it; role_key must name something that will READ the doc — a role on the roster (list_roles), or a member that boots with that role_key (list_members) — or the write is refused 404, so a lessons doc can no longer be created under a name nothing reads and no listing shows; emptying or sharply shrinking it needs allow_shrink=true; and the result is still judged against the lessons cap.",
		...
		},
		{
			...
			Requires: principalAgent,
			Summary:  "Patch a per-role lessons doc by unique anchors ({edits:[{old,new}]}). role_key must name something that will READ the doc — a role on the roster (list_roles), or a member that boots with that role_key (list_members) — or the patch is refused 404.",

...
var workerSharedCoreRewrites = []sharedCoreRewrite{
	...
	// 成立、而且很重要的紀律一起丟掉，所以就地改寫中間那段機制描述。
	{
		...
		Replace: "接手的新你用 MCP `get_my_task` 領回這張任務＋手冊快照，再照 task plan／步驟備註（`update_step_note` 寫的那個）＋ 你上一代留給自己的交接 baton 接回進度——然後**接著跑完**。`get_my_task` 的 steps 是**有界投影**：只有當前節點帶 DoD／備註全文，其餘節點只給編號／名稱／狀態／順序，被省略的字數報在 `steps_omitted_chars`。整份計畫要全文就 `get_task`（不省略）；很大就丟給 sub-agent 消化、別整包塞進自己的 context。",

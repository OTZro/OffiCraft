...
var workerSharedCoreExclusions = []sharedCoreExclusion{
	...
	{
		Anchor: "- ✅ 同型請求第三次出現、還沒有手冊",
		Line:   true,
		Why: "第十處殘留，與上面兩條 ✅ 同級同病理：它是一條**正面示範**，叫 worker 去" +
			"`create_task_manual` 建任務類型、`update_task_manual` 寫手冊 SOP、再請 owner 設負責——" +
			"正好就是 §10.1（「接案／建任務類型是成員治理職責」）與 §10.1b（「手冊維護是成員" +
			"治理職責。worker 只回寫學習經驗，不改手冊設定」）被排除的白紙黑字理由。" +
			"它的兩個同一份清單裡的兄弟（發包、接案三條路）都被排除了，只有它漏掉。" +
			"它沒有 §N 指標，所以交叉引用守衛看不到它；overlay 從頭到尾也沒有任何一句推翻它。",
	},
	{
		Anchor: "- ❌ 手冊寫明這類任務由**別的成員**或**外包**負責",
		Line:   true,
		Why: "第十一處殘留。整行預設讀者是**派工的那一方**：❌ 的情境（「手冊寫明由別的成員或" +
			"外包負責，你卻自己收下來做」）對 worker 根本不會發生——它就是被指派的那一個外包，" +
			"綁死一張任務，沒有「收下來」這個動作。而括號裡是**正面指示**：「你是窗口：轉交負責" +
			"成員，或建好交伺服器喚醒外包」——直接指派身分並叫它轉包，與已排除的 §10.1c" +
			"（「worker 就是被發包的那一方，不轉包」）是同一件事，只是換了措辭、沒有 §N 指標。" +
			"整行刪掉而非就地改寫，是因為刪掉括號後剩下的 ❌ 空殼對 worker 沒有任何可執行意義；" +
			"同一張清單裡的兄弟（scrum master、等待不是停下）也都是整行刪。",
	},
	{
		Anchor: "（**外包負責的任務，負責人就是那個外包工作者本人**",
		Line:   true,
		Why: "第十二處殘留。這個括號整段是**寫給協調窗口讀的**：用第二人稱「你這個協調窗口」" +
			"指派讀者身分，用第三人稱「該外包」「它」講 worker。用第二人稱送給 worker，字面意思" +
...
var workerSharedCoreRewrites = []sharedCoreRewrite{
	...
	// 成立、而且很重要的紀律一起丟掉，所以就地改寫中間那段機制描述。
	{
		...
		Find:    "接手的新 session **先用 MCP `peek_resume_summary_size` 探快照多大**（只回大小／counts ＋ `estimated_total_chars`、不含任何內容），再決定怎麼接回：小（經驗法則的門檻：小於 20000 字元、約 5k tokens）就直接用 `resume_summary` 拿回、大就派便宜 sub-agent（如 haiku）去 `resume_summary` 拉回並回壓縮摘要——然後**接著跑完**。`resume_summary` 快照是**有界摘要**（省你的開機 context）：每張任務只有編號／標題／狀態／優先權／當前節點名稱＋進度，**不含 steps／DoD 全文**。它另外帶兩塊「你醒來的現場」：`roster`（全體成員與外包，各帶線上／離線狀態、所在機器、以及職責——職責截 1000 字（截的是文件扣掉自己標題行之後的內容），結尾 `…` 表示被截；外包沒有角色，改帶他手上那張任務的標題）與 `machines`（機器清單 ＋ `you_are_on` 你自己在哪一台；**「我在哪台」以這個欄位為準，不要用 hostname 推**——我們的機器會互報同名）。`overview` 欄帶大小概要（未結案任務總數、省略掉的計畫文字字數 `detail_chars`、快照 chat 字數 `chat_chars`、你的等回覆卡數，另有 `roster_chars`／`machines_chars` 兩塊現場的字數，peek 讀的就是這塊）——**先看大小再決定**：細節按需 `get_task` 逐張拉，`detail_chars` 很大就丟給 sub-agent 消化、別整包塞進自己的 context；卡片列表用 `list_reply_cards`（有 `limit` 可限制筆數，列表只給標題＋決策要點，全文 `get_reply_card`）。",
	...
	},
	{
		...
		Find: "；**屬你這個角色的**（對你之後做任何事都成立的通則）→ 照 §9 整併進你角色的" +
			"學習筆記（lessons）。兩軌同一個整併紀律：整理不是往後貼，而且**兩軌都吃 §9a 那個一份文件的" +
			"字數上限**（超標而且沒變短就整份不寫，留時間整理）。🔴 **兩軌也共用同一道入場門檻與同一條寫入紀律**：" +
			"①**一句話寫得進去只有一個理由——少了它，下次會做錯事**；②**整份寫回之前一律現讀一次**——" +
			"手上那份副本的有效期只到下一次寫入為止，**拿過期的基底整份覆蓋會靜默刪掉別人後來加的東西**" +
			"（它更短所以過得了上限、非空所以過得了縮小守衛，**沒有任何守衛在檢查你的基底是不是最新的**）。" +
			"ad-hoc 任務無手冊，只有角色這一軌。",
		Replace: "。整併紀律：整理不是往後貼，而且手冊的學習經驗有**一份字數上限**（數字看手冊回應上的 `cap_chars`，owner 可調高）" +
			"（超標而且沒變短就整份不寫，留時間整理）。🔴 **另外兩條對你一樣成立**：" +
			"①**一句話寫得進去只有一個理由——少了它，下次會做錯事**；②**整份寫回之前一律現讀一次**——" +
			"手上那份副本的有效期只到下一次寫入為止，**拿過期的基底整份覆蓋會靜默刪掉別人後來加的東西**" +
			"（它更短所以過得了上限、非空所以過得了縮小守衛，**沒有任何守衛在檢查你的基底是不是最新的**）。" +
			"對你之後做任何事都成立的通則，一樣寫進手冊的" +
		...
		Why: "第二個指向已排除 §9 的 lessons 指標（T-3351 起這句同時指向同樣已排除的 §9a" +
			...
			"它只有手冊那一軌，且不能指向讀不到的章節。T-c4ee 起這句還帶入了兩軌共用的" +
			"入場門檻與「整份寫回前先現讀」——`write_task_learnings` 同樣是整份取代，" +
			"那條紀律對 worker 一字不差地成立，所以 Replace 也照樣帶著它（自足、不指向 §N）。",
	},
}
